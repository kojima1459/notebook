...
' 2026-09-07 R39(レビュー M1): GS 引数に "%04d"(ページ別出力)が入り % の対が
' ずれるため、% を使わない if errorlevel の3値(0/1/255)でフラグへ書く形へ。
'   … & (if errorlevel 1 (echo 1) else if errorlevel 0 (echo 0) else (echo 255)) >"<flag>""
