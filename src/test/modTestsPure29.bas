...
' ----------------------------------------------------------------------------
' (E) modRibbonFail.LooksLikeLimitError - <verdict>タグ救済(R30 W2-2・3件目)。
' ----------------------------------------------------------------------------
...
Private Sub TestE0204DecomposeVerdictRescue30()
    ...
    modTestRunner.Check "R30-W2-2_decompose型verdict応答は救済(誤爆しない)", _
        (modRibbonFail.LooksLikeLimitError(decomposeResp) = False), _
    ...
    modTestRunner.Check "R30-W2-2_verdict=clarifyでも救済", _
        (modRibbonFail.LooksLikeLimitError("<verdict>clarify</verdict>上限の解釈が複数ありうる") = False)
