...
Public Function EnforceExpiry() As Boolean
    ...
    ' ------------------------------------------------------------------------
    ' 2026-08-16(R33H F21・データ喪失): 【消す直前に、もう一度だけ実測する】。
    '
    ' Reachable() の False は「確かに届かない」ではなく「今回の1回では確かめ
    ' られなかった」でもある。冷えたVPN・スリープ復帰の直後は SMB セッション
    ' が未確立で、1回目の GetAttr が数秒かけて失敗しうる(W4-6 の再試行の
    ' 閾値もそこに掛かる)。その1回の偽陰性がそのまま reachableNow=False へ
    ' 流れ、判定は "wipe" ―― 予告文が唯一の解除手段として案内した操作
    ' (社内NWに接続して一度開く)を実行したその瞬間に本棚を失う。
    '
    ' 判定不能で消さない側へ倒すには、消す直前に【キャッシュを使わない】
    ' プローブを1回だけ足すのがいちばん素直で、費用も「一生に一度の消去の
    ' 直前に1回」だけ。届いたなら今日の到達を記録して "none" へ落とし、
    ' セッションの到達判定もやり直させる(ResetProbe。ここで NG のままだと
    ' 共有機能が丸ごと死んだセッションが続く)。
    '
    ' 【本来消すべきケースは従来どおり消える】: 本当に届かない端末は、この
    ' 再確認でも届かないので act は "wipe" のまま。判定式 ExpiryDecision には
    ' 1文字も触っていないので、W2-2 の不変条件(4)と 3,660通りの総当たりは
    ' そのまま成立する(modTestsPure33)。
    ' ------------------------------------------------------------------------
    If StrComp(act, "wipe", vbBinaryCompare) = 0 Then
        If Not reachableNow Then
            Err.Clear
            If modShare.ProbePath(modShare.BasePath(), True) Then
                TouchReach
                act = "none"
                modShare.ResetProbe
                modLog.LogUsage "guard_wipe_averted", "", _
                    "失効の判定は消去でしたが、消す直前の再確認で共有フォルダへ" & _
                    "届いたため中止しました(冷えたVPN・スリープ復帰の直後は" & _
                    "1回目の確認が失敗しうる)。"
            End If
            Err.Clear
        End If
