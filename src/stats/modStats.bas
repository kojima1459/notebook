...
' ----------------------------------------------------------------------------
' AskTotalAll - 質問した回数の総計(すぐ聞く+しっかり調べる+入念に調べる)
' ----------------------------------------------------------------------------
' 2026-08-03(R14-1a / 実機第3報 RC1): 質問回数は modAsk が
' "ask_" & mode & "_total" で加算するのでモードごとに3キーある。ところが
' 合算する側は3箇所(Hubのタイル・テレメトリ・プロンプトの質問者背景)とも
' quick+deep の2キー決め打ちで、入念に調べるで質問した回数が【どこにも
' 出てこない】状態だった(実機の「3回質問したのに1回と表示」の正体)。
' モードを増やすたびに3箇所を直す設計そのものが誤りなので、合算はここ1本に
' する(憲章§4-5: 同型の問題は共通部品で一度だけ解決する)。
Public Function AskTotalAll() As Long
    AskTotalAll = GetStat("ask_quick_total") + GetStat("ask_deep_total") + _
                  GetStat("ask_thorough_total")
End Function
