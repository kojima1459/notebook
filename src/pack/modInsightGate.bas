...
'   True を返したら【送らない】。
' ----------------------------------------------------------------------------
'   【何が抜けていたか】共有フォルダへ出ていく経路は3本ある。
'     ・EmitGap(困りごと)      … PiiBlocked を通していた
'     ・EmitCorrection(訂正)   … 2本の文字列それぞれに PiiBlocked を通していた
'     ・EmitVerifiedQA(解決済みQ&A) … 【1回も通していなかった】
'   しかも3本のうち、これだけが【質問全文・回答全文・実名】の3点セットを運ぶ。
'   他の2本は匿名IDで、質問文か訂正文の片方しか載せない。いちばん重い荷物を
'   運ぶ経路だけが無検査だった。
'
'   【「本人が押したのだから同意がある」は成り立たない】modInsightIo と
'   modInsightGate のコメントは、この経路を「本人が✅解決したを押して共有すると
'   分かっている経路」として実名掲載を正当化している。実名についてはそのとおり。
'   だが同意の対象は【自分のQ&Aを部内に出すこと】であって、
'   【そこに混ざった取引先の携帯番号を部内全員に配ること】ではない。
'   まして回答本文は LLM が生成した文で、利用者が一字ずつ選んだものではない
'   (質問文を復唱する癖があるので、質問に入った個人情報は回答にも移る)。
'   共有フォルダに一度書いたものは取り消せない ―― だから出す前に見る。
'
'   【回答本文も見る理由と、その副作用】本棚の社内資料が根拠なので、長い数字
'   (証券番号・金額)で誤検知する余地はある。それでも見る側へ倒す。取り消せない
'   経路での取りこぼしは、見送りの不便と釣り合わない(憲章§3-6)。誤検知は
'   StripDateLike と同じやり方で後から個別に潰せるが、出てしまった番号は戻せない。
'   見送ったことは必ず利用者へ伝える(NotifySkip "pii_qa")。黙って捨てない。
Public Function QaBlocked(ByVal q As String, ByVal ans As String) As Boolean
    On Error Resume Next
    ' 走査は【実際に送る文字列と同じ内容】を【別々に】。連結すると片方の末尾と
    ' もう片方の先頭の数字が繋がって偽の長い数字列になる(R32 F4 と同じ理由)。
    ' ScanClean を通すのも同じ作法(R32 F17。Clean1 済みを渡すと改行が半角
    ' スペースへ均されて別々の数字列が繋がる)。
    If PiiScanHit(q) Or PiiScanHit(ans) Then
        QaBlocked = True
        modLog.LogUsage "insight_pii_skip", "qa", _
            "個人情報を含む可能性があるため解決済みQ&Aの部内共有を見送りました"
        NotifySkip "pii_qa"
    End If
    On Error GoTo 0
End Function
...
Public Sub NotifySkip(ByVal why As String)
    ...
    Select Case why
        ...
        ' R49(監査 H-7 / H-H-3): 解決済みQ&A専用の2文。
        ' 【なぜ文面が「取り消し」なのか】この経路の呼び出し元 modAsk は
        ' EmitVerifiedQA を呼んだ【直後】に「部内に共有されます」という MsgBox を
        ' 出す。成否を見ずに出しているので、見送り・書込み失敗のときは画面が
        ' 嘘をつく(憲章§3-3)。modAsk は凍結かつ残300字で分岐を足せないため、
        ' こちらのトーストが後から打ち消す形にしてある。
        ' トーストは waitless=True で OnTime 消去なので、モーダルの MsgBox が
        ' 開いている間は消えない ―― 利用者が OK を押したあと最後に残るのは
        ' この訂正文のほうになる。
        ' 【本筋の直し】MsgBox 自体を成否で分ける(=modAsk の凍結解除と分割)。
        ' 書けなかったQ&Aを溜めて次回起動で送り直す「送信待ち箱」も要る。
        ' どちらも層1なので R50 の裁定へ回した(docs/dev/spec_..._R49_裁定表.md)。
        Case "pii_qa"
            msg = "【さきほどの『部内に共有されます』は取り消しです】" & _
                  "個人情報が含まれているかもしれないので、" & _
                  "この解決済みQ&Aは部内へ共有していません。" & _
                  "解決した記録とバッジはそのまま残っています。"
        Case "write_qa"
            msg = "【さきほどの『部内に共有されます』は取り消しです】" & _
                  "共有フォルダへ書き込めなかったため、" & _
                  "この解決済みQ&Aは部内へ届いていません。" & _
                  "ネットワークに繋がってから、もう一度同じ質問をして" & _
                  ChrW(&H2705) & "解決した を押してください。"
        Case Else
