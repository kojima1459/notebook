...
'
' R28H F9(m-5): 20,000 では既定の上限いっぱい(5往復×4,000字)が入らなかった。
' 区切り ";;;" が3字×4本ぶん加算されるため必要量は 5×4,000 + 4×3 = 20,012字
' で、20,000 では5件目が必ず落ちる ―― 「1往復4,000字・5往復まで」と言いながら
' 上限どおりに使うと黙って4往復になる状態だった。20,016 へ引き上げる
' (20,012 に余白4字。6件目は 20,012+4,003 = 24,015字 で依然入らない)。
Private Const MAX_CARRY_TOTAL_CHARS As Long = 20016
