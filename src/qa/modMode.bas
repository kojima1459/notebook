...
' (単一情報源・R48)。
' ----------------------------------------------------------------------------
' リボンへの1回の呼び出しは config llm_wait_sec(既定1800)まで戻らない。さらに
' リボンは wait 超過で同じ要求を【再送】し、回数上限が無い。つまり待ち続けても
' 返らない場面が原理的に存在する。憲章 §3-1「押せるものは必ず反応する」に
' 照らして、無反応のまま放置せず【出口を明示する】。
'
' R48 Fix(敵対的レビュー R48-REV-01): 初版は「30分たっても変わらないときは
' Excel を終了して構いません」と案内していた。**これは誤りだった。**
' 裁定書 §3-6 は「Application.Run の内側に DoEvents が無く ESC の窓が開かない」
' と断定していたが、その grep は【自社モジュールだけ】を対象にしていた。
' 実際に待機ループを回しているのはリボンちゃん側の ChatGPT() で、その待機ループ
' (リボンちゃん_xlam/GPT.bas:379-387)には DoEvents が実在する。
' modAsk.bas:176 が EnableCancelKey = xlErrorHandler を立てているので、
' 待機中の ESC はそこで観測されて Err 18 になりうる。
' CLAUDE.md §3-2「『無い』には探索範囲を書く」そのものの失敗だった。
'
' 「Excel を終了してよい」は、取込中なら本棚が中途半端な状態で残る案内であり、
' 憲章 §3-5(利用者のデータは失敗しても失わない)に反する。ESC を案内する形へ変える。
' **ESC が実Excelで実際に効くかは未確認**(この環境では検証不能)なので、
' docs/09 の R-4 で実機確認する。
