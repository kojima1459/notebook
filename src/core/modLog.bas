...
' PORTAL_FAIL_LEAD(2026-08-16 R33H F32): E0905 の【主因の1文】。
'   FriendlyMessage("E0905") は「主因 + コピーしたので貼り付けてください」の
'   2文構成だが、そのコピー自体に失敗すると modClip.CopyOrGuide が案内文を
'   丸ごと差し替えるので、主因まで一緒に消えていた(画面には「クリップ
'   ボードに入れられませんでした」だけが残り、押したボタンと繋がらない)。
'   失敗時の前置きとして同じ1文を渡せるよう、文言の情報源をここ1箇所にする。
Public Const PORTAL_FAIL_LEAD As String = "社内ポータルをこのパソコンから開けませんでした。"
...
Public Function FriendlyMessage(ByVal code As String) As String
    Select Case UCase$(Trim$(code))
        ...
        Case "E0205"
            ' 2026-09-18(R48): AIリボンは通信に失敗しても例外を出さず、決まった
            ' 文字列を戻り値で返す(HTTPエラー/接続切れ/応答の解釈失敗)。R47 まで
            ' 検査しておらず、利用者には「AIの回答」として表示されていた。
            ' E0202(混雑・操作の重なり)と主因が違うので分けて案内する。
            ' E0204(本日の上限。明日「同期」で再開)へ寄せると嘘になる。
            FriendlyMessage = "AIとの通信に失敗しました(回答は作られていません)。" & _
                "少し時間を置いてから、もう一度お試しください。" & _
                "続くようなら「診断」の結果を管理者へ連絡してください。"
        Case "E0207"
            ' 2026-09-19(R48 Fix・敵対的レビュー R48-REV-01): 利用者が ESC を
            ' 押して止めたときの合図(Err 18)。従来は E0202「通信が混み合って
            ' います」へ丸めており、自分で止めた人に障害だと嘘をついていた。
            '
            ' R48 Fix2(2周目): **この文言は modAsk.bas:249 の Err 18 分岐と
            ' 逐語で同一にしてある。** そちらは ESC が段の境界(DoEvents)で
            ' 観測されたときの経路で、こちらは LLM 呼び出しの内側で観測された
            ' ときの経路 ―― 利用者にとっては【同じ操作】なので、違う案内を
            ' 出してはならない(初版は「質問するボタン」と「そのまま送り直す」で
            ' 指示が食い違っていた)。
            ' **片方だけ直さないこと。** modAsk は凍結モジュールのため今回は
            ' 文言を寄せるに留めた。FriendlyMessage への一本化は R49 で行う
            ' (docs/dev/spec_20260919_R49_裁定表.md)。
            FriendlyMessage = "操作を中断しました。" & _
                "もう一度質問するときは、質問するボタンを押してください。"
        Case "E0206"
            ' 2026-09-18(R48): AIリボンが content_filter に該当したときの応答。
            ' 通信失敗と混ぜない ―― 利用者がとるべき行動が違う
            ' (通信は「待つ」、フィルタは「表現を変える」)。
            FriendlyMessage = "質問または資料の内容が、社内AIの利用ルール(内容フィルター)に" & _
                "該当したため回答できませんでした。" & _
                "表現を変えて質問し直すと通ることがあります。"
        Case "E0301"
        ...
        Case "E0703"
            ' 2026-08-14(R32 W2-3): 実際の動作はOKのみの無条件中止(Yes/No選択肢は
            ' 670ff81で意図的に削除済み)で「続けるかどうか判断してください」は
            ' 実態と矛盾していた。加えてR32 W2-1でこのPII走査自体がconfig
            ' pii_scan_enabled(既定FALSE)の任意機能になったため、その前提も
            ' 文言へ反映する。
            FriendlyMessage = "個人情報チェック(config: pii_scan_enabled)で、" & _
                "書き出す内容に個人情報らしきものが見つかったため、書き出しを中止しました。" & _
                "該当の資料を本棚から外すか内容を修正してから、もう一度お試しください。"
        Case "E0705"
        ...
        Case "E0905"
            ...
            FriendlyMessage = PORTAL_FAIL_LEAD & _
