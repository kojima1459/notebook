...
' インストーラが起動のたびに vba_src シートの本文を VBProject.VBComponents.Add
' + AddFromString で注入する形)は 2026-09-02 に社内AVの AMSI で検知され
...
' 配布物に一切残らない(=ThisWorkbook.VBProject 参照そのものが消える)。
'
...
' CodeModule.Lines で全文を取り出して比較すると巨大な文字列結合が走るうえ、
' VBE側が行末の空白を落とす等の無害な差でも不一致になり、偽陽性を招きかねない
' ため。AddFromString は末尾の空行の扱いが環境で1本ぶれることが知られていた
