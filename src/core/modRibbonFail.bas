...
' ----------------------------------------------------------------------------
' CancelledErr - 「利用者が止めた」を表す1本の文字列(純関数・R49)。
' ----------------------------------------------------------------------------
' R48 はこのリテラルを modGateway・modAskMulti・modAskThorough の3箇所へ
' 手で書いた。IsCancelled は先頭11字("#ERR:E0207:")しか見ないので、
' 文言だけが食い違っても検査には引っかからず、画面に出る言葉だけが
' 段によって変わる ―― 気づける人はいない(憲章§4-5 単一情報源)。
' 作る側も判定する側も、この1本から取る。
Public Function CancelledErr() As String
    CancelledErr = "#ERR:E0207:利用者の操作で中断しました"
End Function
