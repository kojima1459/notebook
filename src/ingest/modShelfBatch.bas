...
' ----------------------------------------------------------------------------
' 保存の機会は「自己インストーラのopen時Save」と「終了ボタン」だけで、取り
' 込んだチャンクとログは数時間メモリ上だけに置かれていた。途中でExcelが落ちれば
' 127分かけた取込が丸ごと消える(憲章§3-5)。1件取り込むごと・同期の末尾ごとに
' 保存し失われる範囲を1件ぶんに抑える。保存は失敗し得る(共有ロック・読み取り
' 専用・容量)ので失敗は必ず1行残し(save_fail)、利用者には次の一手だけ伝える。
' 成功時は無言(毎回トーストを出すと1.1秒×件数の待ちになる)。
' R18-2c: 呼び出し元は modShelf.IngestFile の Finish(成功時のみ・silent含む)へ
' 一本化した。取込の出口が1つになったので、どの入口から取り込んでも保存される。
'
...
Public Sub SaveCheckpoint(Optional ByVal changedN As Long = 1, _
    ...
    Else
        ...
        Err.Clear
        ' R18-2d: 保存が成功した瞬間の (my_knowledge行数, FullName) を控える。
        ' 次の起動でこれと突き合わせて「保存が次回に残ったか」を確かめる。
        If failNum = 0 Then modIntegrity.RecordSaveMark
    End If
