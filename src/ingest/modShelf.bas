...
' ========================================
' modShelf - 本棚中核(ファイル取込・チャンク付番・重複排除・削除・一覧)抽出→チャンク化→chunk_id付番(ハッシュ重複排除)→my_knowledge追記→
' manifest upsert→埋め込み(MASTER_SPEC §7.2)。再入guard(E0503)=mIngesting、
' 出口はFinish一本化(中間保存もそこ=R18-2c)。同名source置換は「書いてから消す」(R18-2e。全重複なら消さない=R18H FA-5)。書込は配列一括(§12)。
' ========================================
...
' IngestFile - MASTER_SPEC§7.2の手順どおり1ファイルを取込む。戻り値=manifest status("done"/"partial"/"failed"/"image_pdf")
' silent (2026-07-28 レビュー L-11): 一括取込で1件ごとにモーダルが出ると同期が朝まで止まる。True なら記録だけ残す。
' outErrCode (2026-07-30・要件E): AddFilesResult 向け失敗理由内訳(E0504/image_pdf/E0302等)の補助出口。Optional末尾追加で既存呼び出し元
'   (modVault/modUIShelf/modShelfSync等)は無改修。成功時は""。
' interactive (2026-08-04 R15-FixA FA-1・A-H1/B-H1):「利用者がその場にいる取込か」。silent(結果モーダル抑止)と別問いを1本で表していたため
'   R15-7bの事前確認が【全経路で死んでいた】。TrueはAddFilesResultと手動同期だけ(R18-6d)。無人同期は従来どおり無確認。
Public Function IngestFile(ByVal path As String, ByVal origin As String, _
    ...
    ' 1) 再入guard(E0503)。焼き付きガードは自動解除。R15-1a: 失効は「開始から」ではなく「最後のビートから30分」(取込途中の二重取込を防止。実機第4報RC5)。
    If mIngesting Then
    ...
    End If
    If mIngesting Then
        ' 2026-07-31(R7 B-2): ここは ShowError(モーダル)だった。再入のたびダイアログが積み上がり実機で「応答なし」に見えた。ログは残し実況行で伝える。
    ...
    mIngesting = True
    mIngestingSince = Now
    ' R15-FixA(FA-6): 中断の印はここでも必ず下ろす(modVault/modUIShelfは直接呼ぶため前回の印が残ると幽霊中断になる)。一括取込・同期は判定済みで来る。
    ...
    If Not extractOk Then
        ' vision フォールバック(画像の文字起こし・画像PDFのOCR)は判断ごとmodShelfVision が引き受ける(2026-07-31 R6)。上限ページ打ち切りは
        ' pagesTruncated が立ちpartial判定へ合流。silentはそのまま渡す(R11-A C4: 無人同期からGSの案内が出ると同期が止まる)。
        ...
        Else
            ...
            If LenB(failMsg) = 0 Then
                ' 2026-08-01(同梱-9): 50MBガード(txt/md/csv入力上限)だけはerrDetailの理由をそのまま使う(E0302の一般文言で上書きすると
                ' 「大きすぎる」が出ない)。他は生ログなので不採用。
                If InStr(errDetail, "ファイルが大きすぎます") > 0 Then
                    failMsg = Split(errDetail, " [")(0)
                Else
                    ' 2026-08-03(R13-3b): 文言決定はmodLog.FriendlyFailMsgへ一本化(docx/docにGhostscriptの話をしない。第2報RC6)。
                    failMsg = modLog.FriendlyFailMsg(errCode, errDetail, modUtil.ExtOf(path))
                End If
            End If
            If isSelf Then
                ' R18-2a: chunk_count は -1(=前値保持)。ここは実データを1行も消していない失敗なので、0で上書きするとカードだけが「0件」に
            ...
            outErrCode = errCode
            ' 2026-08-04(R15-FixB FB-5): 事前確認「いいえ」の資料は【失敗していない】。statusはimage_pdfのまま理由コードだけ差し替え、一括取込の集計で
            ' 見送りを失敗と分けて数える(先頭一致)。
    ...
    Dim chunkN As Long
    ' R14-5a(実機第3報 RC7): 拡張子別の粒度設定(PDF/Word)がここまで届いていなかった。ChunkParamForが「拡張子別→グローバル→既定」の2段フォールバックを1箇所で行う(chunk_modeはグローバルのまま)。
    ...
    ' 2026-07-29: 1ページの失敗でファイル全体を捨てない。ただし黙って減らすと「88チャンクが70」に気づけないので必ず1行残す。
    ...
    If skippedPages > 0 Then
        ...
        ' On Error GoTo 0ではなくFailedへ戻す(0にするとFailedハンドラごと解除され以降の実行時エラーが素通りする=レビューH-3と同型)。
        On Error GoTo Failed
    End If
    ...
    ' 置き換え対象(同名source)の行は未削除なのでハッシュ集合から除外する。除外しないと入れ直すチャンクが全部「自分自身との重複」になる(H-6)。
    ...
    ' R17 Phase1: 構造メタ(section_path/refs_out)。採るのは【ApplyCrumb前】の生チャンク(crumbOn=Falseだと保存本文からbreadcrumb行が消えるため。設計書§3)。
    Dim mPath() As String: ReDim mPath(1 To chunkN)
    ...
    For ci = 0 To chunkN - 1
        ...
        If Not existingHashes.Exists(hashHex) Then
            ...
            ' 保存本文(SafeLeft後)から作る。1セル超は空にし検索側の遅延バックフィルへ委ねる(切り詰めるとスコアが揺れるため)。
            ...
            modChunkMeta.MetaOf chunks(ci).full_text, mPath(acceptedCount), mRefs(acceptedCount)
        End If
    Next ci
    ...
    ' 7.5) 旧データを消すのは【新しい行を全部書き終えてから】(R18-2e)。落ちても旧データが無傷になる順序=§3-5・調査agent1 §3(a)の真の消失経路対策。
    '      firstNewRow は今書いた行を道連れにしないための境界(同名source)。削除失敗時も取込は続けるが、err_log に1行(§4-1)+partial化(FA-6)。
    '      R18H FA-5: 全チャンクが既存と重複(acceptedCount=0)なら削除しない(新規0件で旧行を消すと資料が丸ごと0件になるため)。
    uiStep = "既存同名資料の置き換え"
    ...
    If acceptedCount > 0 Then
        ...
        If rmNum <> 0 Then
            ...
            On Error GoTo Failed
        End If
        ' 削除で行が詰まった分だけ新しい行は上へ動く。読む位置をシートの事実から取り直す(新しい行は必ず末尾に連続している)。
        lastNewRow = wsK.Cells(wsK.Rows.count, 1).End(xlUp).row
        firstNewRow = lastNewRow - acceptedCount + 1
        If firstNewRow < 2 Then firstNewRow = 2
    Else
        ' R18H FA-5(A-M5): 旧行は温存したまま失敗として畳む。従来は素通りしてdone/0件で確定し、旧行だけが消えていた。
        On Error Resume Next
    ...
    ' 7.6) 構造メタをchunk_metaへ1回で追記(R17 Phase1)。失敗は握ってusage_log。
    modChunkMetaStore.WriteMetaFromRows outRows, COL_ID, mPath, mRefs, acceptedCount
    ...
    ' 9.5) バッチ富化。config enrich_modeの既定"off"ならEnrichPendingは即0(2026-07-28 M-15: 呼び出しが1つも無く【死に設定】だった)。
    uiStep = "要約・キーワードの付与"
    On Error Resume Next
    ...
    ' modExtractorPdf.ThinExtractMemoFor。44ページの約款がchunks=1で「登録成功」になる形(第2報RC1)をdoneと言わせない最後の関所。
    ...
    ' カードのメモとpartial判定は別。「前回の続きから再開しました。」だけのことがあり、材料にすると成功資料が「一部だけ」と名乗る。欠けの事実はpagesTruncatedが運ぶ。
    Dim cardMemo As String: cardMemo = thinMemo
    If LenB(cardMemo) = 0 Then cardMemo = visionNote
    ' R18H FA-6(A-M4): 旧行の削除に失敗した回は新旧2組の行が残ったまま「done」と名乗っていた(検索結果重複に利用者が気付けない)。partialへ倒し、自分で直せる唯一の手(もう一度取り込む)をメモに添える。
    ...
    Else
        resultStatus = "done"
        ' 2026-08-04(R15-FixB FB-1・A-M1/B-M): OCRの頁控えを消すのは【ここ】。読み切った瞬間に消すと、後で落ちた資料がpartialのまま控えだけ失い
...
Failed:
    ...
    ' ハンドラ稼働中はOERNが効かず、ここで起きたエラーが呼び出し元へ飛んで本来の原因を上書きする。後始末の前にResumeで抜ける(実機err#462)。
...
FailedCleanup1:
    ...
    If isSelf Then
        ' R18-2a: chunk_countは-1(前値保持)。どの段で落ちたかに関わらず、数え直していないのに0と名乗ってはいけない。実データと台帳のずれはSourceListの突合(R18-2b)が次の描画で必ず直す。
        modShelfStore.UpsertManifestRow path, sourceName, SafeFileDateTime(path), SafeFileLen(path), -1, _
...
Public Sub DeleteSource(ByVal sourceName As String)
    ...
    On Error Resume Next
    ' R18H FA-3: 意図して減らした事実を整合性マークへ即反映(保存はしない。ui_stateはブックと一緒に保存/破棄されるので辻褄は必ず合う)。無いと次の起動で「資料が消えました」の虚偽警告が出る(A-M2)。
...
'   食い違えば【実行数を正】としてmanifestを直す(取込失敗がその列を壊すと実データが生きていても「0件」に見えた)。全読み済みなので追加コスト無し。
        ...
        If lastK >= 2 Then
            ...
            For r = LBound(kArr, 1) To UBound(kArr, 1)
                ...
                If LCase$(Left$(org, 5)) = "pack:" Then
                    ...
                    End If
                Else
                    ' R18-2b: source別の実行数。パック由来は台帳に載らないので数えない。hintは【当たったときだけ】更新(-1に戻すと台帳に無い行が続く間ずっと全走査になる)。
                    Dim mi As Long
...
' IsBusy - 取込が走っているか(2026-07-31 R7 B-2)。DoEventsで発火したクリックを画面遷移側の入口で受け流す判定。焼き付きガードで全ボタン無反応にならぬよう、IngestFileと同じ期限を過ぎたらbusy扱いにしない。
Public Function IsBusy() As Boolean
    ' R10c(H2): mIngestingはIngestFile1件ぶんしか覆わない。ファイル間の隙間もbusyにするため一括取込のバッチガードをORで見る。
...
' ChunkKeyOrder - 拡張子別チャンク設定キーの名前を組み立てる(純ロジック)。2026-08-03(R14-5a/5c): ChunkParamForはmodConfig依存で検証不能だが、
' 「拡張子別キーがbaseKeyより先」の順序はキー名だけ切り出せば固定できる(modTestsPure11)。extは念のため小文字化する。
...
'   1) baseKey_ext(例 chunk_target_chars_pdf)→ 2) baseKey(グローバル既定)→ 3) fallbackDefault。chunk_modeは対象外(仕様どおりグローバルのまま)。
