...
Private Sub ToolbarSpec(ByVal isTable As Boolean, ByVal isShared As Boolean, _
    ...
    ' 1件側は幅を58のまま据え置く ―― 規模差はキャプションの語だけで示す。
    ' 【2026-09-05 R36 で法則が反転した】R33H 当時の「幅を広げると
    ' modTestsPure11『ギャラリーは一覧表より広い』が反転する」は、R36波1で
    ' 📖本文(58)を一覧表側に足した時点で【もう成り立たない】。いまは一覧表の
...
'   WARN帯目前のため実体をこちらへ置く)。白と濃色の実コントラストを比べて大きい方を返す
'   (旧しきい値0.179方式は light primary を濃色と誤判定していた)。
