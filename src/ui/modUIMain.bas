...
Public Sub EnsureLayout()
    ...
    uiStep = "既定フォント設定"
    ' R18-3a: 全域(ws.Cells)への書式はUsedRangeを膨張させる(agent2 §1.3)。
...
Public Sub ShowEmptyShelfHint()
    ...
    modUIMainShape.WriteSafe ws.Range(RNG_ANSWER), _
        ...
        "まず「ナレッジと本棚」で資料を1つ追加してみましょう " & ChrW(&H2192)
End Sub
...
Private Function TipList() As Variant
    TipList = Array( _
        ...
        "資料は「ナレッジと本棚」の「フォルダ→同期」でまとめて自動追加できます。", _
        ...
        "ダッシュボードでは、これまで節約した時間やバッジの獲得状況が見られます。" _
    )
End Function
