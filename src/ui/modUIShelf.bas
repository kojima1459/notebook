...
Private Sub ApplyShelfExtent(ByVal ws As Worksheet, ByVal lastRow As Long, _
    ...
    Dim addr As String: addr = modKnowledge.ApplyShelfBound(ws, bottomY, withFont)
    If LenB(addr) = 0 Then Exit Sub
    If withFont Then
        modBackdrop.RestoreShelfHeaderBg ws, HEADER_ROW
...
Public Sub OnIngestScreenshot()
    ...
    On Error Resume Next
    modTextView.AfterShot destPath, ingestStatus
    On Error GoTo 0
    Exit Sub
Fail:
