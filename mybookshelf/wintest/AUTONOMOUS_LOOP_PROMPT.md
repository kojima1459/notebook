...
あなたはWindows実機上のClaude Codeです。`mybookshelf/` ディレクトリで、
「マイ本棚AI」というExcel VBAツールの実機動作を自動検証・自動修正してください。

**前提知識**(必ず先に読むこと):
- `mybookshelf/docs/dev/MASTER_SPEC.md` — 実装契約書(§3依存ルールR1-R5、§7モジュール契約、§12コーディング規約)
- `mybookshelf/docs/dev/RIBBON_API_CONFIRMED.md` — 社内AIリボンの確定API仕様+PM裁定
- `mybookshelf/wintest/README.md` — このテスト環境自体の説明
...
**やってはいけないこと**:
- `mybookshelf/` 以外のファイルには触れない(このリポジトリには別プロダクト
  `src/chatbot_v2/` が同居しているが無関係)
