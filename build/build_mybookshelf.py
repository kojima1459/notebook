...
def build_config_rows(mock_llm: bool, publish_key: str = ""):
    return [
        ...
        ("onepass_max_chars", 300000,
         "1回読みで載せる本文の合計上限(字)。20万トークン≒30万字。遅いときは150000へ"),
...
def _make_admin_guide(wb):
    ...
    kv("1回に読ませる資料の量(max_context_chars)",
       ...
       " onepass_max_chars(既定300000)を使います。", 0)
    kv("回答が遅い・固まるときの戻し方",
    ...
    kv("入念の1回読みと戻し方(thorough_onepass)",
       ...
       "気になる場合は off にせず、onepass_max_chars(既定300000)を\n"
       "150000 まで下げてお試しください。", 0)
    back_to_toc(toc_header_row)
