...
def _launcher_bat_text(xlsm_name: str) -> str:
    ...
    lines = [
        ...
        "rem 候補)。レビュー R42 M1: D: は日をまたぐと消える場所なので、先に",
        "rem OneDrive(SRC)側にも同じ _旧版 を1本置く(揮発しない退避)。SRC へ",
        "rem 置けない環境では書き戻しも成立しないので、ここで止める。",
        f'copy /Y "%DST%%XLSM%" "%SRC%{upgrade_name}" >nul',
        "if errorlevel 1 goto :COPY_FAIL",
        f'move /Y "%DST%%XLSM%" "%DST%{upgrade_name}" >nul',
        "if errorlevel 1 goto :COPY_FAIL",
        ":COPY_NEW",
        "rem 新版(または初回)。強制上書きで複製し、版の印も一緒に複製する。",
        "rem 印の複製失敗も止める(レビュー R42 M2: 印が無いままだと次回以降",
        "rem 毎回 :RETIRE に入り、退避と全複製を繰り返す)。",
