...
def build_config_rows(mock_llm: bool, publish_key: str = ""):
    return [
        ...
        ("deep_neighbor", 2,
         "「入念に調べる」と「続けて質問(しっかり調べる)」で、根拠チャンクの前後"
         "何個ぶんを一緒に読むか(0=off)。表や条文が途中で切れて『資料に記載なし』に"
         "なるのを防ぐ。増やすほど読む量が増える(1回あたり max_context_chars で頭打ち)"),
        ...
        ("ocr_confirm_min_minutes", 15,
         "画像PDFの読み取りに何分以上かかる見込みのとき、取り込む前に確認"
         "ダイアログを出すか(2026-08-04 R15-7b)。総ページ数×1ページあたりの"
         "実測時間(実測が無い端末は25秒/ページ)で見積もる。前回の途中まで"
         "読めているページは待ち時間に数えない。0にすると確認せずに始める"),
        ("vision_pdf_dpi", 150,
