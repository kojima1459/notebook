...
_README_TEXT = (
    ...
    "\n"
    "【1. 開き方】\n"
    "  1. この zip ファイルの中身を、すべて【OneDrive のフォルダ】\n"
    "     (消えない場所)へ展開(解凍)してください。\n"
    "  2. 展開してできた「MyBookshelfを起動.bat」をダブルクリックしてください。\n"
    "     D: ドライブへの複製と、閉じたときの OneDrive への書き戻しは\n"
    "     bat が自動で行います。特別な操作は要りません\n"
    "     (D: にすべて置いて使う場合も、従来どおりそのまま動きます)。\n"
    "\n"
    ...
    "     起きません。\n"
    "  ★ 保存には3つの段階があります。1)Excelで保存する\n"
    "     (取込中の自動保存以外は手動保存が必要) 2)bat がD:から\n"
    "     OneDriveのフォルダへ書き戻す(開いている間は10分ごと、\n"
    "     閉じたときは閉じてから約30秒後) 3)OneDriveがそれをクラウドへ\n"
    "     同期する。この3)まで終える前にPCの電源を切ると、その分の\n"
    "     作業が消えることがあります。作業が終わったら Excel を閉じ、\n"
    "     bat と一緒に開いた黒い画面(コマンドプロンプト)が自動で消える\n"
    "     まで待ってから PC をシャットダウンしてください(この画面は\n"
    "     2)の書き戻しが終わると自動で消えます。手動で閉じたり ×で\n"
    "     消したりしないでください。3)のクラウド同期は別途OneDriveの\n"
    "     アイコンでご確認ください)。\n"
...
_README_DOCS_GENERAL = (
    "【同梱の手順書】\n"
    "  docs\\05_図解ガイド.html              … ★まずこれ。図で分かる1枚の説明書\n"
    "                                          (ダブルクリックでブラウザが開きます)\n"
    ...
    "  docs\\06_部門ナレッジの配り方ガイド.html … 部門の正典を配る/受け取るときの手順\n"
)
...
# (OneDrive原本 → D:複製 → 書き戻し)を追加。R36(a)の前倒し。司令塔レビュー
# (2026-09-04)で3点直したのが現行の形(下記に反映済み)。
...
#   ・bat 自身の場所(%~dp0)が原本フォルダ。SRCがD:上に展開された運用
#     (D:\MyBookshelf\に限らず、D:上のどのフォルダでも。2026-09-04
#     差し戻し2回目・司令塔指摘1)なら複製も書き戻しもせず従来どおり
#     起動して即終了する(ループを作らない)。D:上のデータはどのみち
#     シャットダウンで消える場所なので、そこから更にD:\MyBookshelf\へ
#     複製する往復自体に意味が無い。SRCの先頭2文字(ドライブレター+
#     コロン)だけを%SRC:~0,2%で取り出し/iで比較する。この判定は
#     「入口」プロセスだけで行う。
#   ・D: が無い端末では複製先を作れないので、メッセージを出さず従来どおり
#     SRCから起動して即終了する(D:前提でない端末を壊さない)。ドライブの
#     存在確認は `if not exist D:\nul` を使う(`if not exist "D:\"` は
#     環境によって判定が揺れることがある定番の落とし穴のため避ける)。
#   ・入口プロセスが黒い画面のまま残ると非エンジニアが誤って閉じる
#     (司令塔レビュー指摘1)。そこで顔を3つに割る: 入口(即終了)→
#     start /min "…同期中(閉じないでください)" "%~f0" __loop__ で複製・
#     監視・書き戻しを最小化した別プロセスへ丸投げ→その__loop__からも
#     start /min "…待機中" "%~f0" __wait__ でExcel起動/終了待ちだけを
#     さらに別の最小化プロセスへ切り出す。__wait__を__loop__と同じ
#     プロセスでstart /waitするとバッチ本体がExcel終了までブロックされ、
#     監視ループが1回も回らなくなるため分離が必須。
#   ・SRC/DST/XLSMはどの顔でも%~dp0等から同じ値を再計算できるので、
#     プロセス間の引数受け渡し(旧設計の%2 %3の引用符問題)は無い。
#   ・監視は30秒ごとに.closedを見る(司令塔レビュー指摘2: 10分ポーリング
#     だとExcelを閉じてから最終書き戻しまで最大10分待たされる)。
#     中間の書き戻し(コピー)自体は監視20回=10分に1回だけ間引いて行う。
...
#     解釈されるため %ERRORLEVEL% や カウンタ変数を素の書き方のまま
#     安全に読める(このループのN・RETRYも同様)。
#   ・コピーの成否判定は %ERRORLEVEL% を変数展開せず `if errorlevel N` の
#     コマンド形で見る(遅延展開の落とし穴を踏まない)。
#   ・ディレクトリの存在確認は末尾に \ を付けて書く(`if exist "%DST%"` 等。
#     DSTは末尾\の定数)。付けないと同名ファイルとの誤マッチが起きうる。
# ---------------------------------------------------------------------------
...
def _launcher_bat_text(xlsm_name: str) -> str:
    ...
    R35 F3b(司令塔レビュー反映・2026-09-04、2回差し戻し済み): OneDrive
    (SRC=bat自身の場所)とD:(DST=固定の複製先)を往復する。SRCがD:上
    (D:\\MyBookshelf\\に限らずD:上のどこでも)・D:が無い、の2ケースは
    複製せず従来どおり起動して即終了する(ループを作らない)。

    顔は3つ:
      1) 入口(利用者がダブルクリックした最初のプロセス)。判定だけして
         即終了する。SRCがD:上かD:が無ければここでExcelを起動して
    ...
    upgrade_name = _upgrade_source_name(xlsm_name)
    # R42 2周目 m4: OneDrive 側に置く「前版の控え」。手置きの _旧版(R36 §1-A)と
    # 衝突しない別名。引継ぎは D: 側の _旧版 だけを見るので、控えは人が拾う用。
    _stem, _ext = os.path.splitext(xlsm_name)
    keep_name = f"{_stem}_旧版_控え{_ext}"
    ...
    lines = [
        ...
        "rem SRCがD:上に展開された運用(どのフォルダでも。D:\\MyBookshelf\\に",
        "rem 限らない)なら複製も書き戻しも行わず従来どおり起動して即終了する",
        "rem (ループを作らない)。D:上のデータはどのみちシャットダウンで",
        "rem 消える場所なので、そこから更にD:\\MyBookshelf\\へ複製する往復に",
        "rem 意味が無い。SRCの先頭2文字(ドライブレター+コロン)だけを",
        "rem /iで大文字小文字を無視して比較する。",
        'if /i "%SRC:~0,2%"=="D:" (',
        ...
        "rem ドライブの存在確認は D:\\nul の exist 判定を使う(ドライブ直下の",
        "rem 予約デバイス名nulの有無を見る定石。\"D:\\\" だけの判定は環境に",
        "rem よって結果が揺れることがあるため使わない)。",
        "if not exist D:\\nul (",
        '    start "" excel.exe /x "%SRC%%XLSM%"',
        "    goto :EOF",
        ")",
        ...
        "rem 印の比較は cmd 組み込みの set /p だけで行う(レビュー R42 M3: fc.exe は",
        "rem 外部コマンドで、塞がれると 9009 が「版違い」に化けて毎回退避する)。",
        "rem .build は ASCII 1行なので set /p で丸ごと読める。読めなければ空同士",
        "rem にならないよう片方を消しておく=不一致=退避側へ倒す。",
        'set "SB="',
        'set "DB="',
        'set /p SB=<"%SRC%%BUILD%"',
        'set /p DB=<"%DST%%BUILD%"',
        "rem 空同士(0バイトの印)も一致=同版として扱う(2周目 m7: 空を退避側へ",
        "rem 倒すと毎起動 :RETIRE になる)。",
        'if "%SB%"=="%DB%" goto :COPY_LEGACY',
        "goto :RETIRE",
        "",
        ":RETIRE",
        "rem 版が違う。D:側の本体(利用者データ入り)を _旧版 へ退避する(引継ぎ",
        "rem 候補)。先に D: 内で move し、成功してから OneDrive(SRC)側へ「控え」",
        "rem を別名で置く(レビュー R42 M1/2周目 m4・m5: D: は日をまたぐと消える",
        "rem 場所なので揮発しない控えを1本置く。SRC の手置き _旧版 は潰さない。",
        "rem 控えが置けなくても起動は続け、案内窓で手作業を促す)。",
        f'move /Y "%DST%%XLSM%" "%DST%{upgrade_name}" >nul',
        "if errorlevel 1 goto :COPY_FAIL",
        f'copy /Y "%DST%{upgrade_name}" "%SRC%{keep_name}" >nul',
        "if errorlevel 1 goto :RETIRE_WARN",
        "goto :COPY_NEW",
        ":RETIRE_WARN",
        f'start "MyBookshelf 前版の控え失敗" cmd /c "echo 前の版の控えをOneDrive側に置けませんでした(OneDriveの空きと同期状態を確認してください)。& echo %DST%{upgrade_name} を手で安全な場所へコピーしてください。& pause"',
        ":COPY_NEW",
        "rem 新版(または初回)。強制上書きで複製し、版の印も一緒に複製する。",
        "rem 印の複製失敗は止める(レビュー R42 M2/2周目 m6: 印が無いままだと",
        "rem 次回以降毎回 :RETIRE に入り、_旧版 と控えを新版で潰してしまう。",
        "rem 案内は印の失敗と分かる文言にする)。",
        ...
        ":COPY_DONE",
        "rem どの経路でも、複製先に本体が無ければ起動しない(R42 司令塔 Fix:",
        "rem xcopy が 1=対象なし で戻る/元が無い場合の取りこぼし)。",
        'if not exist "%DST%%XLSM%" goto :COPY_FAIL',
        "",
        ...
        "rem 複製できたかを事後に確かめる(レビュー R42 m4)。欠けていても Excel は",
        "rem 起動する(画像PDF以外は使える)が、黙って E0303 に化けないよう案内する。",
        f'if not exist "%DST%{gs_dir}\\gsdll32.dll" goto :GS_WARN',
        f'if not exist "%DST%{gs_dir}\\gswin32c.exe" goto :GS_WARN',
        'if exist "%DST%.gs_warned" del "%DST%.gs_warned"',
        "goto :GS_DONE",
        ":GS_WARN",
        "rem 案内は1回だけ(2周目 m8: 恒久的に欠けている環境で毎起動窓が出ない",
        "rem よう印を置く。複製に成功したら印を消して、次に欠けたときはまた出す)。",
        'if exist "%DST%.gs_warned" goto :GS_DONE',
        'type nul > "%DST%.gs_warned"',
        'start "MyBookshelf Ghostscript 複製失敗" cmd /c "echo 画像PDF読み取り用のGhostscriptをD:へ複製できませんでした。& echo 通常の資料は使えます。画像PDFの取込は次回起動時にもう一度試します。& pause"',
        ":GS_DONE",
        "",
        ...
        "rem 遅延展開が不要な素直な形にするため。カウンタもset /aで素の",
        "rem まま読む(遅延展開setlocal enabledelayedexpansionは使わない)。",
        'set "N=0"',
        ":COPY_LOOP",
        ...
        "if not errorlevel 1 goto :WB_LOG_OK",
        "rem R43 3-10(spec_20260910_R43 §3-10): 定期書き戻しが失敗した時刻を",
        "rem 残す。従来はここで記録が一切残らず、いつから書き戻せていない",
        "rem のか(この起動からか・数時間前からか)を後から調べようが無かった。",
        'echo %date% %time% > "%DST%.wb_ng"',
        ...
        "",
        ":WB_LOG_OK",
        "rem R43 3-10: 成功した時刻も残す。.wb_ng と付き合わせれば、",
        "rem 「いつまでは正常で、いつから失敗しているか」を後から追える。",
        'echo %date% %time% > "%DST%.wb_ok"',
        "goto :COPY_LOOP",
        "",
        ":FINAL_COPY",
        ...
        "",
        ":FAIL",
        "rem __loop__自身は最小化ウィンドウ(タイトルも「閉じないでください」)",
        "rem なので、ここでpauseしても利用者には見えない。通常サイズの新しい",
        "rem 窓を開いてそこにメッセージを出す。%DST%%XLSM%はこの行(ブロックの",
        ...
        "",
        ":BUILD_FAIL",
        "rem 2周目 m6: 本体は複製できたが版の印を置けなかった。印が無いと次回",
        "rem 毎回 :RETIRE に入るため、ここで止めて原因(D: の書込み)を確認してもらう。",
        'start "MyBookshelf 版の印の複製失敗" cmd /c "echo 版の印(%BUILD%)をD:へ複製できませんでした。& echo D:\\MyBookshelf の書き込みができるか確認してから、もう一度起動してください。& pause"',
        "goto :EOF",
        "",
        ":WAIT_AND_MARK",
...
def build_dist_zip(xlsm_path: str, dist_dir: str, is_publisher: bool, is_dev: bool,
    ...
    doc_names = [
        ...
        "10_使い方ガイド.md",
        "44_P2P実機テスト手順.md",
        # R43 §3-7: 部門ナレッジ配信は配る人・受け取る人の両方が使う機能な
        # ので、発行者用に限らず全員のzipへ同梱する(46は発行者用のみ)。
        "06_部門ナレッジの配り方ガイド.html",
        # R45: R41〜R45 で直した箇所の受入確認手順。45(スモークテスト)は R11
        # 当時の一般確認で、この5ラウンドで変えた箇所は1つも載っていない。
        # 「何を・どう動かし・何を見て・あるべき結果は何か・異常は何か」を
        # 優先度順に並べる(上から順にやれば危ないものから潰せる)。
        #
        # R47(2026-09-13): 07/08/09 の3本を同梱すると、テスターは「どれを
        # やればいいのか」から判断させられる。実機受入が未消化なのは R46 と
        # R47 の2ラウンドで、09 がその両方を1枚に統合している(07/08 の項目は
        # すべて 09 へ取り込み済み)。**同梱は 09 の1本だけにする**。
        # 07/08 は docs/ に履歴として残す(doc_gate の検査対象のまま)。
        "09_受入確認_R46_R47.html",
    ]
    if is_publisher:
...
def build_config_rows(mock_llm: bool, publish_key: str = "", build_stamp: str = None):
    ...
    return [
        ...
        ("recommended_model", "gpt-5.6-terra", "精査モード(🔍しっかり調べる)で使うモデル名(R48でgpt-5.5から変更。リボンの一覧が5.5を「5.6-terraを使用ください」と降格した)"),
        ("quick_model", "gpt-5.6-luna", "即答モード(⚡すぐ聞く)で使うモデル名。検索の拡張/再ランク段・取込の章要約と名寄せ・一般アシスタントも空欄時はこれを使う(速さが価値の段なので軽いモデルにする)"),
        ("thorough_model", "gpt-5.6-sol", "入念モード(📚入念に調べる)で使うモデル名(R48新設)。4段/1回読み/論点分解/俯瞰の全経路に効く。【空欄にしても既定の gpt-5.6-sol のままで、何も変わらない】。しっかり調べると同じにしたいときは gpt-5.6-terra と書くこと"),
        ...
        ("llm_wait_sec", 1800, "ChatGPT() 呼び出しのWait秒数(既定1800)。これは【1回の呼び出し】の上限で、1質問の上限ではない(入念の論点分解は最大11段)。この秒数を過ぎるとAIリボン側が同じ要求を再送する(回数上限なし)ため、短くすると重複請求になりやすい"),
        ...
        # R38(入念の1回読み): 単発経路(複合質問の分解に落ちない回)を、章丸ごと
        # 読み+1回のLLM呼び出しに置き換える(modAskOnePass)。失敗時は黙って
        # 従来の4段(要点整理→下書き→自己点検→検証)へ落ちる。
        ("thorough_onepass", "on",
         "「入念に調べる」を1回読み(関連する章の本文を丸ごと1回で読んで回答)にする。"
         "off=従来の4段(要点整理→下書き→自己点検→検証)"),
        ("onepass_max_chars", 450000,
         "1回読みで載せる本文の上限(字)。【合計上限ではない】 ―― 本文ブロックと重点抜粋ブロックがそれぞれ独立にこの値で切られる(R48実測: 実効最大は約473,500字≒31.6万トークン)。遅いときは300000へ"),
        ("onepass_max_chapters", 8, "1回読みで読む章の最大数"),
        ("onepass_seed_hits", 6,
         "章の候補にする検索上位の件数(onepass_max_chaptersより小さいときは章数まで引き上げる)"),
        ("onepass_neighbor", 6,
         "章に割れていない本棚での代替(当たったチャンクの前後を何件ずつ読むか)"),
        ("thorough_onepass_effort", "high", "1回読みのreasoning_effort"),
        ...
        ("freeze_keep_banner", False,
         "TRUE=取込・入念な質問応答などの長時間ブロック中、DWMの「応答なし」白画面化"
         "(ゴースト化)をuser32.DisableProcessWindowsGhostingで抑止し、進捗バナーの"
         "文字を最後まで見えるようにする(R16-2b)。"
         "【R46で既定をFALSEへ変更】抑止するとウィンドウの移動・最小化・×閉じが"
         "効かなくなることは既知だったが、逃げ道として案内していた『タスクマネー"
         "ジャーから強制終了』も同時に効かなくなる。ゴーストウィンドウはOSが"
         "無応答アプリへ用意した唯一の復旧経路で、タスクバーからの復元・右クリック"
         "→閉じる・タスクの終了はすべてそこを通るため。R45実機で『取込中に最小化"
         "したらPC再起動以外に手が無い』が発生し、xlsmのロックも残って再実行が"
         "読み取り専用になった。バナーの見た目より復旧手段を優先する。"
         "TRUEにすると旧挙動(バナーは見え続けるが復旧不能)へ戻る"),
        ("nexus_share_path", "",
        ...
        ("active_channel", "",
         "【2026-09-11 R46で説明を訂正】直近に選んだ部門チャンネルの控え。"
         "旧説明は「いま接続している部門(1つだけ)。切り替えると前の部門は本棚から外れる」と"
         "書いていたが、実装は modChannel.SubscribeAllAvailable が見つかった全部門を購読する"
         "(既定で全チャンネル購読・R13の設計)。購読の可否を決めているのは下の "
         "unsubscribed_channels だけで、このキーは購読範囲に影響しない"),
        ...
        ("log_max_rows", 4000,
         ...
         "R37で2000から倍にした: 1回の質問が ask と ask_sources の2行を書くようになり、"
         "同じ2000行で振り返れる期間が半分になったため(ask_sources は正誤と出典を突き合わせる測定の材料)"),
        ...
        ("confidence_score_x100", 55,
         "【2026-09-11 R46で未使用になった】旧・信頼度バッジのスコア閾値×100。"
         "この値以上のヒットが2件以上で「強く一致」という判定だったが、0.55は埋め込みの"
         "絶対スケールに依存し、固有名詞が一致するだけで超えていた(実機で「三井住友の株価」に"
         "🟢が出た)。しきい値が1本しかないため△も構造上ほとんど出なかった。"
         "R46以降のバッジは下の conf_* 3本(スケール非依存のcoverage/flatness)が決める。"
         "このキーはもうどこからも読まれない(残置は旧configとの互換のためだけ)"),
        # R46 A-4: 確信度をスケール非依存の比だけで決める(実体は modMode.ConfidenceOf)。
        # 実測(教えてBOX 10,996かけら・「火災保険(法人)」440かけらを抜いて同分野の
        # 実質問87件を投げる/答えがある質問362件と比較)での3値の出方:
        #   答えがある  🟢69.6% 🟡29.0% 🔴 1.4%
        #   答えが無い  🟢 5.7% 🟡12.6% 🔴81.6%
        ("ground_check", True,
         "TRUE=回答に出てくる数字(金額・第N条・％・桁区切りの数など)が、渡した資料と"
         "質問文の中に実在するかを照合し、見つからなかったものを注記で挙げる。"
         "信頼度バッジは「検索が材料を取れたか」までしか見ておらず、AIがその材料どおりに"
         "書いたかは別の事実なので、点数に混ぜずに注記として別に出す。"
         "実測(2026-09-11・上位5件だけを根拠に正解回答250件を照合): 指摘が出たのは5.2%で、"
         "中身はいずれも上位5件に本当に無い値だった。実在しない金額を1つ混ぜると100%検出。"
         "FALSEで照合そのものを止める"),
        ("conf_cov_green_x100", 100,
         "🟢(根拠あり)に必要な語の充足率×100。質問から抜いた「効く語」のうち、"
         "採用チャンクに実在する語の割合。100=全部の語が出てくること。"
         "「三井住友の株価」は株価が本棚に無いので33になり🟢にならない"),
        ("conf_flat_green_x100", 160,
         "🟢に必要な「1位の突出度」×100。1位スコア÷上位の平均。"
         "答えがあるときは1位が突出し(実測の中央180)、無いときは全部が横並びの"
         "中くらいになる(同117)。比なので埋め込みのスケールが変わっても意味が変わらない"),
        ("conf_cov_amber_x100", 90,
         "🟡(部分的に一致)に必要な語の充足率×100。これを下回ると🔴(根拠が乏しい)。"
         "90より下げると、答えが無い質問に🟡が出やすくなる"),
        ("holidays", (
        ...
        # R36波1(2026-09-05 §3): 📸スクショ取込の直後に、読み取り結果(text_view)を
        # 自動で開くかどうか。FALSEにすると従来どおりトースト表示のみに戻る
        # (エスケープハッチ)。📖本文ボタン自体はこのフラグに関係なく常に出る。
        ("screenshot_show_text", True, "TRUE=📸スクショ取込の直後に、読み取った文章の確認画面(📖本文)を自動で開く。FALSEで従来どおりトーストのみ"),
        # 2026-09-05 R36 §2(是正の仕組み): ❌違う→「正しい内容」で作られた
        # 是正メモを、次から同じ質問の回答で必ず先頭の出典にする。注入は
        # modCorrect.InjectHits が modAskRetrieve.RunDeepScoped/RunUnscoped の
        # 出口で1回だけ行う(凍結モジュールは不触)。off にすると R35 以前と
        # 同じ「是正メモも普通の資料の1冊」に戻る=誤爆したときの逃げ道。
        ("correct_inject", "on", "on=❌違うで登録した是正メモを、質問が一致したとき回答の出典の先頭へ必ず差し込む。offで従来どおり普通の資料として検索されるだけになる"),
        # 質問が完全一致しないときの救済。DistinctiveKeys(最大8語)のうち何%が
        # 是正メモの質問行に現れたら「同じ質問」とみなすか。高いほど誤爆が
        # 減り、低いほど拾いやすくなる。60=5語の質問なら3語一致で通り2語では
        # 通らない。100にすると実質「完全一致だけ」になる。
        # R36 Fix A-m4: 0 は「語一致を使わない(完全一致だけ)」として受け付ける。
        # 従来は 0 を「壊れた設定」とみなして既定60へ黙って戻していたため、
        # 誤爆を嫌って 0 にした人の設定が無視されていた(modCorrect.KeyMinPct)。
        ...
        # 【配線の事実・graph_synonyms と同型】リンクづくりは章要約の配線に
        # 相乗りしている(modOutlineBuild.BuildOutlineFor の末尾で1行呼ぶ)。
        # そこへ到達するには graph_outline=on で、chunk_meta があり章キーが
        # 1つ以上取れる必要がある。つまり graph_outline=off の本棚と、
        # 章・条の見出しが取れない資料では xdoc_links=on でもリンクは1行も
        # 増えない。設計書ではなくこの事実を説明文へ書いておく(憲章§3-3)。
        ("xdoc_links", "on",
         ...
         "【前提】つながりづくりは章の要約の直後に走るため、graph_outline が off の"
         "本棚と、章・条の見出しが取れない資料では on にしてもつながりは増えない"
         "(質問側は従来どおり動く)。できた件数は usage_log の xdoc_built で確認できる"),
        ("xdoc_min_sim", 80,
...
def _make_howto(wb):
    ...
    # 2026-09-05(R36波1 §3): 読み取った文章をその場で確かめるボタン。
    # 📸スクショ取込の直後にも自動で開く(config screenshot_show_text=TRUE)。
    kv("📖 本文", "いま選んでいる1件の、読み取った文章を全部表示します(④で詳しく説明します)。", 1)
    kv("🗑 1件(赤いボタン)", "いま選んでいる1件だけを本棚から削除します(④で詳しく説明します)。", 0)
    ...
    section("読み取った文章を確かめる(新設)", "355C86")
    kv("📖 本文ボタン", "📋 マイ本棚(一覧表示)で資料の行をクリックしてから「📖 本文」を押すと、\nAIが読み取った文章を全部その場で表示します(ページ番号つき)。", 0)
    kv("📸 取込直後は自動表示", "スクショ取込の直後は、読み取り結果の画面が自動で開きます。\n合っているかその場で確認できます。", 1)
    note("読み取った文章を直接書き直す機能はありません。内容が違うときは、その資料を「🗑」で削除してから「➕ 登録」で正しい文章を登録し直してください。")
    ...
    err_rows = [
        ...
        # 2026-09-18(R48): E0205/E0206 を追加。AIリボンは通信に失敗しても例外を
        # 出さず決まった文字列を戻り値で返すため、R47 まではそれが「AIの回答」と
        # して画面に出ていた。握るようにした以上、番号を引ける表にも載せる。
        ("E0205 質問がエラーで返る(通信の失敗)",
         "原因: AIとの通信そのものに失敗した(回答は作られていません)。\n対処: 少し時間を置いてもう一度。続くなら「診断」の結果を管理者へ。"),
        ("E0207 操作を中断した",
         "原因: 待ち時間中にESCキーが押された(障害ではありません)。\n対処: もう一度質問するときは、質問するボタンを押してください。"),
        ("E0206 内容フィルターで回答できない",
         "原因: 質問または資料の内容が社内AIの利用ルール(内容フィルター)に該当した。\n対処: 表現を変えて質問し直すと通ることがあります。"),
        ("E0301 ファイルを追加できない",
...
def _make_admin_guide(wb):
    ...
    kv("1回に読ませる資料の量(max_context_chars)",
       ...
       "使われる共通の設定です。上げると取り込みも重くなる点にご注意ください。\n"
       "※「入念に調べる」の1回読み(thorough_onepass=on のとき)はこの値ではなく"
       " onepass_max_chars(既定450000)を使います。こちらは【合計の上限ではなく】、\n"
       "本文ブロックと重点抜粋ブロックがそれぞれ独立にこの値で切られます\n"
       "(R48実測: 1回の送信は最大およそ473,500文字)。", 0)
    kv("回答が遅い・固まるときの戻し方",
    ...
    kv("入念の1回読みと戻し方(thorough_onepass)",
       "「入念に調べる」は今回の版から、関係する章の本文をまとめて1回で読んで\n"
       "答える方式が既定(thorough_onepass=on)です。configシートの\n"
       "thorough_onepass を off にして保存し、ブックを開き直すと、以前の\n"
       "4段階(要点整理→下書き→自己点検→検証)の動きに戻ります。遅いだけが\n"
       "気になる場合は off にせず、onepass_max_chars(既定450000)を\n"
       "300000 や 150000 まで下げてお試しください。", 0)
    back_to_toc(toc_header_row)
...
def assert_guard_thresholds(rows):
    ...
    # 2026-09-11 R46: 信頼度バッジは confidence_score_x100 を読まなくなった
    # (スコアの絶対値ではなく coverage/flatness という比で判定する)ので、
    # この検算の対象からも外す。加点がバッジを突破する経路そのものが消えた。
    # 低関連度警告(low_hit_warn_score)は依然としてスコアの合計を絶対値と
    # 比べるため、こちらの検算は残す。
    for name, thr in (("low_hit_warn_score", warn),):
        if max_boost >= thr:
            raise SystemExit(
