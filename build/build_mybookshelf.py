...
def _make_howto(wb):
    ...
    kv("⚡ すぐ聞く / 🔍 しっかり調べる", "すぐ聞く=10〜20秒のふだん使い。しっかり調べる=40〜60秒かけて、念入りに調べて答えを検証してから返します。", 1)
    ...
    kv("🗑 削除(右端の赤いボタン)", "選んだ資料を本棚から削除します(④で詳しく説明します)。", 1)
    ...
    note("「検索」は🃏 ギャラリーのときだけ、「🗑削除」は📋 マイ本棚(一覧表示)のときだけ出ます。\n"
         "「📤正典を発行」「📊利用状況」は発行の権限がある端末だけに、「📸スクショ取込」は画像解析が使える環境だけに出ます。")
    back_to_toc(toc_header_row)
    ...
    kv("フォルダは1階層だけ", "「📂 フォルダ」で決めたフォルダの、直下に置いたファイルだけを読みます。\nサブフォルダの中までは読みに行きません。取り込みたいファイルは\nフォルダの直下(サブフォルダの外)に置いてください。", 1)
    ...
    kv("削除ボタンが効かない",
       "「🗑 削除」は📋マイ本棚(一覧表示)で、削除したい資料の行をクリックしてから\n"
    ...
    kv("ボタンが出ない",
       "「📸スクショ取込」は画像解析が使える環境(config feature_vision)だけに、\n"
       "「📤正典を発行」「📊利用状況」は発行キー(publish_key)が設定された端末だけに\n"
       "出ます。どちらも「押しても断られるボタン」を見せない設計です。", 0)
...
def _make_admin_guide(wb):
    ...
    kv("「発行者」と「組織管理者」は別物",
       ...
       "資料の組織的な除外を解除できる人)は、名前は似ていますが完全に別の\n"
       "権限です。片方を持っていても、もう片方の操作はできません。", 0)
    note("configシートを直接編集して自分で発行者になる方法はありません。誰でも発行者になれてしまうと、"
         "部門の正典を誰でも書き換えられる状態になるため、この手順はどこにも書きません。")
    back_to_toc(toc_header_row)
    ...
    note("💡みんなの困りごとは誰でも見られます。📊利用状況だけが発行キーのある端末に表示されます。")
    back_to_toc(toc_header_row)
    ...
    kv("合言葉が違うと言われる", "発行キーは大文字・小文字を区別した完全一致で照合されます。半角/全角の\n"
       "違いや余分な空白にも注意して、落ち着いてもう一度入力してください。", 1)
