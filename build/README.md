...
このディレクトリは `dist/MyBookshelf.xlsm`(または開発版
...
既定の出力先: `dist/MyBookshelf.xlsm`(`--prod`)/
`dist/MyBookshelf_dev.xlsm`(`--dev`)。
...
- `--root PATH` — `modules.json` 内 `path` の解決基準ディレクトリ(既定:
  リポジトリルート)。スモークテスト等で本物のソースツリーに触れずに検証したい
  時に使う。
...
を検証できます。実ソース(`src/`)や `dist/` には一切触れず、
