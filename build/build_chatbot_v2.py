...
def _make_howto(wb, chunks=None):
    ...
    sections = [
        ("基本の流れ（質問→回答→自己理解→評価で1セット）",
         ...
         "6. 「🟢スッキリ解決／🟡もう少し知りたい／🔴まだ不安」で素直に反応（ここまでで1セット）\n"
        ...
        ("🟢🟡🔴 の反応が大事な理由（正誤判定ではありません）",
         "「合ってる/間違ってる」を判定するのではなく、『自分にどう届いたか』を素直に押すだけ。\n"
         "  🟢 スッキリ解決   : その質問と回答が『役に立った例』として保存され、次回似た質問で優先参照\n"
         "  🟡 もう少し知りたい: そのまま『続けて質問』に繋がり、深掘りできる（学習プールは汚しません）\n"
         "  🔴 まだ不安・要確認: どこが不安かを記録し、必要なら『教えてBOX』で本社照会に繋がる\n"
         "  ✏ 正しい答えを知っている: 正しい内容を登録すると、次回から優先参照（任意・分かる人だけ）\n"
         "押すほどボットが賢くなります。気軽に🟢🟡🔴を押してください（迷ったら🔴でOK）。"),
