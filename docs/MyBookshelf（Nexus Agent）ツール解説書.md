...
> 💡 **この章のポイント** — このファイルの起動処理（R35 で方式Bへ転換）。マクロ有効化のみが利用者に必要な設定。起動処理は途中で失敗しても全体が止まらないよう、段階ごとに保護されている。

### 3.1 起動型ブック（R35 で方式B へ転換）

起動は Excel が自動実行する **`modBoot.Auto_Open`** が `modBoot.Boot` を呼ぶ（R35 で方式B へ転換。ThisWorkbook は窓リサイズの転送だけを持ち、`Workbook_Open` は置かない＝二重起動の防止）。ビルド時に `build/ovba_write.py` が全モジュール 155 本を完成品 `vbaProject.bin` へ焼き込んでいるため、実行時の注入は不要。
...
| `modCluster` | 265 | 資料のクラスタリング。球面 K-Means → 資料ごとに多数決でクラスタ ID（分析 CSV 用）。可視化「ナレッジ地図」は R18-4 で撤去 |
...
| 容量上限 | `chunk_limit`（既定 20,000）/ `shelf_max_chunks`（既定 20,500）。超過時は `E0501` |
...
├──────────────────────────────────────────────────────────────┤
│ ┌──────────┐┌──────────┐┌──────────┐┌──────────┐              │
│ │節約した時間 ││登録ナレッジ数││蔵書チャンク数││  レベル   │  KPIカード×4  │
│ └──────────┘└──────────┘└──────────┘└──────────┘              │
│ ▓▓▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░░░░░  EXP進捗バー                    │
...
| # | 見出し | 主値 | 副値 | 算出元 |
|---|---|---|---|---|
| 0 | 節約した時間 | `SavedMinutesEstimate()` を「n時間m分」形式へ | **前月比**（▲n% / ▼n% / 新記録 / —） | 前月比は usage_log の `feedback_green` 件数 × 15分を今月と先月で比較（`CountUsageEvent`）。前月0件かつ今月ありなら「新記録」 |
...
「節約した時間」の換算は **1自己解決 = 15分**（`MINUTES_PER_SELFSOLVE`）。`modStats` 側は Private のため `modDash` に同値が複製されている旨がコメントに明記されている。
...
#### 6.5.5 資料のクラスタリング（`modCluster`）

**2026-08-05（R18-4）にダッシュボードの「ナレッジ地図」（Shape 円の可視化）は撤去した。** 押しても何も起きない見るだけの絵に対し、画面を開くたびに K-Means を再計算し `my_vectors` を最大約 800 回のセル単発読みで舐めていた（キャッシュなし）。さらに共有フォルダ未設定の既定状態ではベクトルが `MIN_POINTS`（8本）に届かず、多くの端末では案内テキストにしかならなかった。オーナーからの「地図はいらない」という指摘に合わせ、可視化専用コードだけを削除している。

クラスタリング自体は**ガバナンス分析 CSV のクラスタ ID 列**が現役で使っているため残っている:
...
4. 資料ごとに、所属チャンクの多数決でクラスタ ID を決める（SourceClusterMap）
```

`SourceClusterMap()` は分析 CSV から呼ばれる（K-Means を内部実行するため、行ループの外で1回だけ呼んでキャッシュする設計）。ベクトルが 8本未満なら空の辞書を返し、CSV のクラスタ列は空欄になる。
...
到達経路が無いため実質的に死んでいるが、起動時（`modBoot`）と資料追加・質問後（`modUIMain` / `modUIShelf`）に今も描画処理が走る。内容は統計タイル4枚（今月の質問数 / 🟢自己解決した回数 / 節約した時間 / 本棚の資料数）＋バッジ棚＋蔵書数バー（`■` の文字列で描くバー）。
...
> 💡 **この章のポイント** — 品質確認の仕組み。計算ロジック部分は Excel が無い環境でも自動テストできるよう分離されており、実機では「「❓ ガイド」→「🩺 診断を開く」」ボタンで利用者自身がツールの状態を確認できる。
...
| 実機自己診断      | UI の「「❓ ガイド」→「🩺 診断を開く」」ボタン → `diag_report` シート                            | シート存在・config キー・本棚統計・直近エラー                                                                                                               |
...
**【解決】** 上記 #4（VBOM 信頼設定）は **R35（2026-09-03）でビルドが完成品 `vbaProject.bin` を生成する方式へ変更**したため不要に。利用者に必要なのはコンテンツの有効化のみ。VBOM の設定変更を情報システム部門に求める必要は無くなった。
...
| キー | 決めるべきこと | 未設定時の挙動 |
|---|---|---|
| `nexus_share_path` | 部内の誰もが読み書きできる UNC パス。**全部門の正典がこの1つの場所の下にぶら下がる**（`channels\<部門名>\`）。発行側と受け取り側は同じ場所を指す必要がある | 現在はダミー値。配信・P2P 機能が全て無効（アプリは動く）。テストだけならローカルフォルダでも可 |
...
| `knowledge_expire_days` | 共有フォルダ最終到達からの失効日数（既定0）。**導入初期は 0（無効）を推奨** | パス設定済み・未到達の端末で 30 日後に知識が全削除される。§11-10（到達できていても削除される）は 2026-07-28 に修正済み |
| `user_department` | 分析 CSV の部署比較用 | 空なら未設定。利用者に入力させるか、部署別ビルドで焼き込むかの判断が必要 |
| `telemetry_enabled` | 利用統計の送信可否（既定 ON） | §12.8 の周知義務と連動 |
| `chunk_limit` / `shelf_max_chunks` | 本棚の上限（既定20,500） | 上限到達時は `E0501` |
| `mock_llm` | 本番は FALSE | TRUE のままだとダミー応答が返る |

| `embed_transport` | 埋め込みの通信経路。**既定は `ribbon`**（2026-07-28 に `direct` から変更） | 本番ビルドは `azure_embed_key` を焼き込めない（配布＝キー配布になるため）ので、`direct` のままだと毎回 ribbon へフォールバックし、err_log が埋まって本当の障害が見えなくなる。`direct` は**キーを自分で config に入れる管理端末**で明示的に切り替えて使う |

> **【解決済み】** 旧版にあった「`embed_transport` を `direct` にするか `ribbon` にするか」の【要決定】は、B1（キーを配布物へ焼き込めない）が決着したことで自動的に決まった。**一般配布は `ribbon`**（＝リボンちゃん経由）。リボン経路は実装済みで、`direct` 側が使えないときの自動フォールバック先でもあるため、**リボンちゃんが使えなくなったわけではない**。`direct` は大量取込を速くするための選択肢で、キーを持つ管理端末に限って使う。
