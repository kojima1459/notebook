...
| **処理が止まった**と見なすまでの時間 | **120秒** | `config` の `vision_pdf_timeout_sec` |
...
- `vision_pdf_timeout_sec` は**「120秒で打ち切る」ではありません**。ページが1枚でも
  進んでいるあいだは待ち続け、**まったく進まなくなってから**この秒数で見切ります
  (2026-08-03に意味を変更)。分厚い資料が途中で打ち切られることはなくなりました。
...
- 待っているあいだは画面のバナーに「本文抽出中… 12/44ページ (経過 35秒)」のように
  今どこまで進んだかが出ます。動いているのに止まって見える、ということはありません。
