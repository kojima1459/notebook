...
**B（bat）**: 版の印 `MyBookshelf.build`（= config `build_stamp`・zip 同梱）。SRC/DST の印を `set /p` で比べ、違えば D: の本体を **SRC と D: の両方に `_旧版`** として退避してから新版を強制複製（引継ぎは既存の初回案内に任せる。「いいえ」は二度と聞かない R36 契約のまま＝データは `_旧版` 2本に残る）。同版は従来の `/D`。起動番号 `SID` 付きの `.closed.<SID>`。各コピーの成否検査（退避失敗は書き戻し中止・定期書き戻し失敗は1回案内）。Ghostscript は exe+dll 両方を見て、複製後も再確認。`tools/check_launcher.py`（7項目・ネガティブ確認済み）を final-gates に追加。
...
**レビュー**: 1周目（Opus）MAJOR 3・MINOR 9 → 採用 M1〜M3・m4〜m6・m8（Fix `688f352`・司令塔）／記録 m1（オーバーラップ起点の窓が前ページ扱い＝仕様どおり・既定値で 14.88%）・m2・m3・m9。2周目（Opus・Fix 検証）: 下記「検問」欄。
**検問（09-09・司令塔）**: lint ERROR 0／LO compile **173/173**／pure **PASS 3292 / FAIL 0 / SKIP 15**（下限 3292・上限 15。ネガティブ確認: Pure45 ゴールデンを "1,2,2,3,5" に壊して FAIL 1→復元）／`check_launcher` OK（(6)(7) をネガティブ確認）／`--dev`・`--prod --zip` 自己検証 OK（173本）／zip に `.build` があり config `build_stamp` と一致。
