# 再開手順（セッション中断対策・最終更新: 2026-09-19 R49 クローズ時点）
...
## 0R49. R49（検問の修理と外向きの実害・2026-09-19）

**裁定書: `docs/dev/spec_20260919_R49_検問の修理と外向きの実害.md`（必読）。**
**憲章は R49 で §1/§2 を改訂済み**（目的＝照会業務をゼロにする／おしえてBOX の
利用者数／自己成長・自己改善／§1-2 相棒・UI/UX は要件）。判定基準はこちら。

### いまどこ

- **実機受入は R46・R47・R48・R49 をまとめて未実施。** 手順書は
  `docs/09_受入確認_R46_R47.html`（R48 で **G-4**・**R群**、R49 で **S群6項目**を追記済み）。
- 実機テストの投げ方は **2本のプロンプトに起こしてある**:
  - `docs/dev/prompt_20260919_R49_1_Antigravity環境構築.md`（**先にこちら**。
    AIリボン環境を作り、**3モデルとも `ChatGPT()` が応答するか**を確かめる）
  - `docs/dev/prompt_20260919_R49_2_ClaudeCode実機テスト.md`（S群・R群＋**D3実験**）
- R48 は単独でクローズせず **R49 へ合流**した（ユーザー裁定「どうせ治すなら閉じる意味ない」）。

### このラウンドで変えた大きいもの

1. **検問が空振りしていた穴を3つ塞いだ（このラウンドの本体）。**
   いずれも「検査は存在し、ERROR 0 を返し続けていた」。
   - `vba_lint` の孤児Public救済が **docs/dev の記録文書まで見ていた** ――
     裁定書に名前が出た Public は未接続でも永久に救済される。範囲を絞ったら
     **隠れ孤児5件**（`modBitwiseOpt.SkippedByDimension` と
     `modBoard.OrgSummaryText` を削除。後者は組織サマリ文言の死んだ3つ目の複製）
   - `doc_gate` が **bool と str（モデル名）を1文字も見ていなかった**。
     さらに **表形式の config 台帳（MASTER_SPEC §5・約60行）を1行も見ていなかった**
     （「既定値」が表の見出しにしか無いため）。足したら実在のずれ5件
   - `doc_gate` の配布文書UI検査が **綴りを1通りしか見ていなかった**。
     語順の両向きを見る形にしたら **7件（4文書）**
2. **容量台帳を機械が書く形にした。** 検査を足した初回、台帳は **35件ずれていた**。
   原因は不注意ではなく構造で、§12 は日付つきスナップショットが4枚重なった状態
   （同じモジュールが3か所に3つの違う数字で載る）。
   **`python3 tools/doc_gate.py --write-cap` が実測から1枚を書き、`--only cap` が検査する。
   人は書かない。** 副産物: 旧台帳に一度も載っていなかった WARN 直下が2本
   （`optGsTxt`/`modRagParse`）、`modClarify` は残233 ではなく**残32**。
3. **製品で唯一、個人情報の関所を通らない外向き経路を塞いだ（監査 H-7）。**
   共有フォルダへ出る3経路のうち `EmitVerifiedQA`（解決済みQ&A）だけが1度も
   走査していなかった。しかも **3本のうちこれだけが質問全文・回答全文・実名の
   3点セットを運ぶ**。`modInsightGate.QaBlocked` を新設。
   **陰性の反例7件を必ずテストに置いた** ―― 過検知は部内ナレッジの
   フライホイール（憲章§1の目的そのもの）を静かに止める。
4. **✅解決した の共有失敗を黙って捨てるのをやめた（監査 H-H-3）。**
   ただし **これは暫定**。`modAsk` が凍結かつ残300字で MsgBox を成否で分けられず、
   **トーストが後から打ち消す形**にしてある（`waitless`=OnTime 消去なので
   モーダルの間は消えず、OK のあとに残るのは訂正文）。本筋は §「次にやること」へ。
5. **ノイズ報告の既定ボタンが「報告」だった（監査 A-A-3）。**
   `vbDefaultButton2` ＝ [いいえ] ＝ ⚠️ノイズ報告。**Enter を押しただけで**
   資料が自分の検索から消え、共有フォルダへ1票飛ぶ。既定を[キャンセル]へ。
   取り消しも新設（`modStats.UnreportNoise` ＋ `modP2P.RetractNoiseVote`。
   従来あったのは管理者専用の `ClearNoise`＝その資料への全員の票を消す操作だけ）。
   実体は新設 **`modNoiseReport`**（`modVaultGallery` 残271 → 残459）。
6. **起動中は見えないタブで画面を切り替えろ、と案内していた（監査 G-G-2）。**
   `DisplayWorkbookTabs = False` なのに、同梱文書4本7箇所と製品の文言3箇所が
   タブ操作を書いていた。製品側は `「ナレッジと本棚」` という実在のボタン名へ。
7. **中断文言を1本へ。** `"#ERR:E0207:…"` が3箇所に手書きされていた
   （`IsCancelled` は先頭11字しか見ないので文言だけ食い違っても検査に掛からない）。
   → `modRibbonFail.CancelledErr()`。

### 敵対的レビュー2周が見つけたもの（**この節が一番読む価値がある**）

**1周目の MAJOR 4件はすべて司令塔の欠陥、2周目の MAJOR 3件はすべて Fix 波が作った
新しい縫い目だった。** §8 が言うとおりの型が再現した。詳細は裁定書 §7-2。

いちばん重かったもの2つ:

- **「部内に共有されます」は出荷既定では今も嘘をついていた。** 書込み失敗の `Else` を
  足して直したつもりが、`EmitVerifiedQA` は `WriteShared` の手前に無言の `Exit Sub` を
  3本持っていて、**出荷既定（共有パス未設定）ではそのうち1本がいちばん通る道**だった。
  足した通知は一度も発火しないまま画面は毎回断言していた。**直したのは最後の一区間だけ。**
- **検査を足したつもりで減らしていた。** 表の見出し判定を「1列目の語」の許可リストで
  書いたため、`docs/46` の `| 設定 | 既定 | … |` という表が丸ごと対象外になり、
  **`pii_scan_enabled`**（§3-3 の一番高くついた嘘の当事者キー）を落としていた。
  → 見出しは「次の行が区切り行か」で判定。比較対象は 40 → 48 → **62行**。

**学び: 検問は回帰を止めるもので、設計の欠陥は止めない。**
7件のうち lint・LO・doc_gate が捕まえたものは1件も無い。

### 検問の記録

`doc_gate` **ERROR 0 / WARN 0** ／ `vba_lint` **ERROR 0 / WARN 59**（182ファイル。
WARN が 53→59 なのは孤児救済の自己無効化を外して隠れていたものが出たため）／
LO compile **OK** ／ LO pure **PASS 3516 / FAIL 0 / SKIP 15**（ベースライン 3507→3516）。
ネガティブ確認: S1 を壊して FAIL 1 → 復元／表の値を `gpt-5.5`・`1536`・`英語`・
`pii_scan_enabled=オン` に壊して検出 → 復元／`--write-cap` をマーカー逆順で rc=1・無変更。

### 次にやること（R50・優先順）

1. **実機受入**（①→② のプロンプトを投げる）。特に **S-2（関所が効きすぎていないか）**と
   **D3実験（30,000字の壁が baked 方式で実在するか）**。
   D3 が「壁は無い」なら、§12 の分割裁定・受け皿という設計上の制約が丸ごと不要になる。
2. **送信待ち箱**（書けなかった Q&A を溜めて次回起動で送り直す）。層1。
   共有フォルダが一時的に届かないだけで組織の知恵が消えるのは憲章§1 に正面から反する。
3. **`modAsk` の凍結解除と分割** → 「部内に共有されます」を成否で出し分ける（層1）。
4. **`modAskRetrieve` の ESC**（拡張・再ランク段）。**残94字**で1文も入らず、
   「中断した」を呼び出し元へ返す戻り値の設計が要る。分割が先。
   実害は「ESC をもう一度押す必要がある」で、重い段（sol・最大30分）は R48 Fix3 で塞ぎ済み。
5. 監査 78 件のうち MEDIUM/LOW（裁定表 `spec_20260919_R49_裁定表.md`）。

## 0R48. R48（モデル3段化・リボン失敗語彙の集約・埋め込みベクトル流用の是正・2026-09-18）

**裁定書: `docs/dev/spec_20260918_R48_モデル3段化とリボン失敗語彙.md`（必読）。**

### いまどこ
実機受入は **R46・R47・R48 をまとめて未実施**。手順書は `docs/09_受入確認_R46_R47.html`
（R48 で **G-4** と **R 群**を追記済み）。

### このラウンドで変えた大きいもの
1. **リボンの失敗文字列を回答として表示しなくなった（最重要）。**
   アドインは通信に失敗しても例外を出さず【文字列を戻り値で返す】。ver202609 の
   `log.bas` を読んで全数（5種）を確定し、`modRibbonFail`（**新規・core**）へ集約した。
   `(error:NNN)` / `接続切れ` / `content_filterに該当しました` /
   `レスポンスから当該テキストを抽出できません…` / `レスポンスのJSON文字列が途中で終了しています。`
   **R47 まではこれが「AIの回答」として確信度バッジと出典つきで表示されていた**（憲章 §4-1/§3-3 違反）。
   判定は**部分一致を使わない**（完全一致 or 先頭一致+数字）。R21 D2・R30 W2-2 の誤爆を3度目にしない。
   エラーコードは **E0205（通信の失敗）/ E0206（内容フィルター）を新設**。429 は既存の E0204 のまま
   （`modEnrich` の取込打ち切りが E0204 系に乗っているため）。
2. **`modGateway` の分割裁定を実行した。** `LooksLikeLimitError`/`LooksLikeRealAnswer` を
   `modRibbonFail` へ移設（**ロジックは1文字も変えていない**）。**残125 → 残2,344 へ回復**。
   参照は本番4箇所・テスト14箇所を同じ変更の中で張り替えた（§9）。
3. **埋め込みベクトルの流用を止めた（本題外・重い）。** `modGateway.RibbonEmbedRange` は
   VBA の `Dim` がループ内で1回しか実行されない性質により、**本文が空のかけらへ直前の
   かけらのベクトルを書き込み、成功として数えていた**。無関係な質問に「似ている」と
   判定されて出典に出るうえ、ログに痕跡が残らない。`Erase v` を入れ、あわせて
   `modEmbed` 側で「空のかけらは失敗に数えない・印だけ立てる・`embed_empty` を記録」
   を同じ波で入れた（片方だけだと3連続失敗で取込が止まる）。
4. **モデルを3段化した。** `thorough_model`（新設）。出荷既定は
   `quick_model=gpt-5.6-luna` / `recommended_model=gpt-5.6-terra` / `thorough_model=gpt-5.6-sol`。
   入念の**4経路**（4段・1回読み・論点分解・俯瞰）だけを `thorough_model` にした。
   **`modAskRetrieve` の expand/rerank は3モード共通経路なので触っていない**（裁定書 §3-3）。
5. **`llm_wait_sec` 1200 → 1800。** ただしこれは**1回の呼び出し**の上限で、1質問の上限ではない
   （論点分解は最大11段）。**うち側に打ち切る手段は無い**（`Application.Run` の内側に ESC の窓が無い）。
   ユーザー裁定（案A）により、実況末尾へ出口を明示する `modMode.WorkingNote()` を新設し、
   4箇所に散っていた literal を単一情報源へ寄せた。
6. **文書の嘘を3件潰した。** 共有ナレッジの自動消去日数が「30日」と書かれていたが
   **出荷ビルドの実測値は 0（＝機能は無効）** /
   必須キー「9個」→**11個** / `onepass_max_chars` の「**合計**上限」→
   **合計ではない**（本文と重点抜粋がそれぞれ独立に切られる。実効最大 約473,500字）。
   README の実装との食い違いも直した（モードは2つではなく3つ＋一般／「即座に変換」は分〜十分／
   「出典が必ず…安心して業務に使えます」は🔴のラベル変更と常設ガードに反する）。

### 次のセッションが知っておくべき落とし穴
- **`doc_gate` はモデル名を検査しない**（`check_config_defaults` が int 値だけを見る）。
  「検問が通った＝文書は正しい」と判断してはならない。R48 の検問には
  `grep -rn 'llm_wait_sec\|gpt-5\.' docs/ README.md CLAUDE.md` の手動確認を含めた。
- **入念と「しっかり」の分離は `modAsk.AnswerWithContext` の分岐順序だけで保たれている。**
  `modMode.UseVerify` は thorough でも True を返す。順序を変えると入念が静かに降格する。
  `modTestsPure47` の R20〜R22 がこの戻り値を固定している。
- **`modAskRetrieve` は残94字。** もう1文字も入らない。次に触るなら分割が先。
- `EXPECTED_PASS_MIN` は **実走した値へ**更新すること（静的カウントは 433 件過小）。

### 見送り（記録のみ・裁定書 §7）
`onepass_max_chars` の見直し（実機の `chars=` を測ってから）／`focusBlock` の予算配分／
リボン呼び出しの打ち切り機構（層1・設計から）／全経路の `chars=` 記録／
どのモデルで答えたかの記録（現状 usage_log に一切残らない）／
`modSynonymStore.bas:191` の無音 Exit Sub ／
`gpt-6-astra` と `gpt-5.6-sol(xhigh)` が luna のデプロイを指している件（**アドイン側の不具合**。開発元へ報告）。
...
2026-09-14 にまとめて確認予定）。手順書は **`docs/09_受入確認_R46_R47.html`**。

**2026-09-13 の配布物更新（コード変更なし・文書と鍵のみ）**
- `docs/09_受入確認_R46_R47.html` 新設（17,322字）。**07/08 の項目を全部取り込んだ
  統合版**で、群は P(最小化・Defender) / A(バッジと出典) / **G(数字の実在照合・新規)**
  / **E(絵文字 FE0F・新規)** / B(前回報告9件) / C(Excel 入れ直し) / **D(正典発行・新規)**
  / Z(おまけ)。zip 同梱は **09 の1本だけ**にした（`build_dist_zip` の `doc_names`
  から 07/08 を外した）。3本入れるとテスターに「どれをやるか」を判断させてしまう。
  07/08 は `docs/` に履歴として残す（doc_gate の検査対象のまま）。
- **発行者用の合言葉（`publish_key`）をユーザー指定の新しい値へ変更**して再ビルド。
  値は文書・コード・コミットのどこにも書かない（`MYBOOKSHELF_PUBLISH_KEY` で焼き込む）。
  実測: 一般配布 `publish_key` 長さ0／発行者用のみ設定あり。
  09 の D-1〜D-3 が「新しい値で通る・古い値で落ちる・一般配布にボタンが出ない」を確認する。
...
**起点**: R44 の6視点解剖で敵対的検証を**生き残った3件**と、ユーザー裁定2件（①一般モードのガードは「指示だけ入れる」②Excel の構造は「次ラウンドでやる」）。全裁定 = `spec_20260910_R45_引き継ぎのガードとシートを章に.md`。

**R45-A（一般モードへ引き継いだ文脈のガード）**: 一般モード（壁打ち）の指示文には**事実面のガードが1本も入らない**（`modApp.bas:191` が `extraRules` に空文字）。そこへ橋渡し（`modConvBridge`）が社内資料由来の本文を流し込むが、**R44 の常設ガード ※ も低関連度 ⚠ も含まれない**（どちらも `modAsk.bas:283` で**その後に**足す表示専用の飾りだから）。画面は「社内資料は未使用」なのに中身は社内回答、という食い違いが最大5往復続く。→ `modGenPipe.CarryRules()`（純関数・新設）を **橋渡しヘッダーがあるターンだけ** `modAppState` から1行で足す。**文言は反証班の要件どおり**「社内資料は一切使えない」とは書かず（`conv_bridge` 既定 TRUE なので直前に出典付きで確定した数字まで撤回させる＝R29 W1-2 と同型）、「会話に出ている情報を根拠として使うこと自体は問題ない」を明示し、止めるのは**断定して言い直すこと**だけ。判定は `modConvBridge.HasBridgeHeader`（新設・`Left$` ではなく `InStr`。1往復進むとヘッダーが途中へ移るため。前後2つの印を両方見るので誤検知しない）。ヘッダー後半を `HEADER_TAIL` として単一情報源化。**凍結モジュール不触**。

**R45-B（Excel の「シート＝章」）**: `ClassifyLine` は Excel の行を章にも節にも一度も分類しない（2セル以上＝タブで表行4／値1個＝`[A5]` 前置で本文0）。結果**ブック全体が1ブロック**になり `section_path` が必ず空 → 章要約・俯瞰・`doc_links`・参照展開・条番号保証が **Excel では全滅**していた。→ シート1行目を **`# シート: 名前`**（`modExtractorExcel.SheetHeadingLine`・純関数）にして章を立てる。`【…】` を使わない理由は `StripHeadingMark` が `#` しか外さず breadcrumb が二重括弧になるため。

**効果の実測（正直に）**: 教えてBOX ナレッジ5ファイル・43シート・10,996かけらで旧新を比較。**点で当てる検索の精度は動かない**（伏せた実データ 上位5 66.0%→66.0%／口語 上位5 62.5%→62.5%／かけら 10,987→10,996）。効くのは (1) **俯瞰質問が Excel でも成立**（章要約が作られる） (2) **かけらがシートをまたがなくなる**＝旧挙動ではシート境界を跨いだかけらの尾部が**別シートの番号で出典表示**されていた（全体の0.4%程度） (3) breadcrumb にシート名（分野名）が入る。**代償**: 取り込み時に「章の仕上げ」が**シート1つにつきAI1回**走る（5ファイル計43回・上限60章/資料）＝**取り込みが長くなる**。**配布済みの部門ナレッジは入れ直しが必要**。プレビューに `#` が見える（軽微）。

**検問**: lint ERROR 0／LO compile 178／pure **PASS 3420 / FAIL 0 / SKIP 15**（下限 3406→**3420**）。**ネガティブ確認**: `SheetHeadingLine` を旧表記 `[シート: 名前]` に戻すと **G7a/G7b がちょうど2件落ちる**（PASS 3418 / FAIL 2）→復元して3420で緑。追加テストは G7（7件）＋G8（7件）。

**容量（python len 実測・R45 後）**: `modConvBridge`残13,125 / `modGenPipe`残6,873 / `modAppState`残9,769 / `modExtractorExcel`残10,286 / `modTestsPure48`残11,766。

**実機で見てほしいこと（spec §4）**: (1) Excel を**入れ直す**と「章の仕上げ」がシート数ぶん走る (2) 「〇〇の全体像を教えて」に Excel 資料からも答えられる (3) 出典の「シートN」が実際のシートを指す (4) 社内ナレッジ→一般モードへ切替後、社内固有の数字を**言い直すときだけ**断り書きが付く (5) 橋渡しをしていない普通の壁打ちには**出ない**。

---

## 0R44. R44（安全装置の復活と常設ガード・2026-09-10・**検問全緑・実機受入待ち**）

**起点**: ユーザーの問い「本当に製品側が効くの？ 出力プロンプトまで変えるのはどうなの？ 他に影響でない？ 木を見て森を見ずな気がする」。全裁定 = `spec_20260910_R44_安全装置の復活と常設ガード.md`。

**§0 撤回**: 司令塔が前便で出した「回答プロンプトに結論先行・根拠・相談先を足す」提案は**空提案だった**。`modPrompts` は既に `StyleInstruction`【結論先行】【構成】、`CitationInstruction`、`NotFoundInstruction`、`DomainGuardInstruction`【数値の厳格性】【(要確認)】【断定の禁止】、`GroundingInstruction`（出荷 `strict_grounding=True`）を全部持っている。**ユーザーの指摘が正しい。**

**§1 本当の足かせ（実測で特定）**: 検索スコアは `類似度 + KeyScore*0.06`（`modRetrieve.bas:280`）。安全装置はその**合計**を絶対値と比べる（⚠ `low_hit_warn_score`=0.3 / バッジ `confidence_score_x100`=55）。ところが `sparse_keyscore_cap` 旧既定 **10** → **加点だけで 0.600** に達し両方を無条件突破。**実測（10,996かけら・質問60件）: 加点だけで0.55を超えるかけらが中央403個（最小22）→ 🟢が100%点灯／⚠は0%発火。** これが「答えが無い質問の38%が答えがあるときと同じスコアで1位に出る」の機械的な正体。資料の種類にもモードにも依存しない**構造欠陥**で、プロンプトでは直せない（表示側の事実誤認だから）。既存コードも `modAskRetrieve` の分散判定で「キーワード加点そのものの遮断は未達（**要裁定・報告済み**）」と書いており、**その未裁定の宿題**だった。

**§2 裁定**: (1) **`sparse_keyscore_cap` 10→3**。3×0.06=**0.18** で 0.3 にも 0.55 にも届かない＝安全装置が生き返る。**反証を試み、成立しなかった**: 専門用語1語での救済は上限3でも**100%維持**（0にすると41.7%へ崩壊＝加点自体は必要）。一般質問の順位も悪化なし（伏せた実データ300件・上位5: 上限10で66.3%→上限3で68.7%）。上書きインストールでも届く（`modMigrate` はホワイトリスト方式で `CFG_KEEP_KEYS` に無い）。(2) **常設ガード ※**（ユーザー要望）を**プロンプトではなくコード**で末尾に付与（`modMode.GuardNoteText`・8pt muted・config `answer_guard_note` で消せる）。**末尾**なのは、先頭だと `LeadParaIndex` が毎回ガードを掴んで画面で一番大きい文字が注意書きになるから（R43 の ⚠/📜 で実際に起きた型）。(3) 表示専用の注意書きを **`modMode.DisplayNotes`（純関数）** へ一本化（⚠ と ※ の順序・重複排除・有効無効）。`modAskRetrieve.ApplyLowHitWarning` は config 読みと最高スコア算出だけへ縮退（**凍結モジュールは不触**）。

**§2-4 プロンプトの穴（§0 の訂正・6視点の並行解剖で発見・司令塔が全件を一次情報で再検証）**: `DomainGuardInstruction` は Quick と4段には入っているのに、**`modAskOnePass`（精査モードの既定経路。`thorough_onepass` 既定 on。成功時は4段へ行かない）と `modAskGlobal`（俯瞰）には1文字も入っていなかった**。＝**いちばん外したくないモードだけ数値の捏造ガードが外れていた。** `modPrompts.DomainGuardInstruction` の**可視性だけ** Private→Public にし（本文は不変）、両経路から1行で呼ぶ。文言の複製は R38 F7 の裁定と矛盾するので採らない。**新しい指示は1文字も足していない**＝一般モード・壁打ちへの副作用なし。
...
**実機で見てほしいこと（spec §5）**: (1) 回答の末尾に小さく `※ AIが資料から作った回答です。…` が毎回出る (2) 結論の12pt太字が従来どおり効いている（ガードに奪われていない） (3) 本棚に無いことを聞くと 🟢 のままではなく 🟡/🔴 に落ちる (4) 専門用語1語で従来どおり当たる (5) 着せ替えを変えてもガード文が読める。

---

## 0R43. R43（UI/UX ポリッシュ・Excel の番地・説明書・軽微修正・2026-09-10・**検問全緑・実機受入待ち（R41/R42 分も未実施）**）

**起点**: R42 配布物の実機報告・要望・質問（09-10）。ユーザーの方針は「**もう大きな修正は加えない。受入で未修正のうち簡単に直るものだけ。あとは UI/UX のポリッシュで、見やすく・認知負荷を下げ・没入感と学習効果を高める**」。全裁定 = `spec_20260910_R43_UIUXポリッシュと軽微修正.md`（§0 質問への回答・§1 波A・§2 波B・§3 波C・§4-1 波D・§4-2 レビュー裁定・§5 実機確認・§6 R44 送り）。裁定の材料は**調査波 11 本＋正典調査 1 本**（読み取り専用・実測）。

**§0 質問への回答（実装不要）**: (1)「章の仕上げが X/6」は**正常**（6 は固定値ではなく、章を「最低3チャンクになるまで合体」させた後の数。M ≦ チャンク数÷3+1）。ただし表示語が誤解を生むので語を直した。(2) **取込元を移動すると次の同期で本棚から消える**（削除と同じ。パスを追う仕組みは無い。ボタンは出ないので壊れない。抜粋は残る）。(3) **Hub の「ご意見」は共有フォルダの `feedback` へ匿名で保存**（出荷既定は共有パス空なので届かず、クリップボードへ入って終わる）。

**波A（チャット系 UI）**: `modLive` の装飾一式を **`modLiveStyle`（新規）** へ分割（27,825→23,214 に回復）。**結論段落の 12pt 太字**（`LeadParaIndex`。`■` 見出し・聞き返し 💭・**⚠️ 関連が薄い**・**📜 前回の回答** の4経路は除外＝レビュー M1）。**`(A6 付近)` を出典タグと同じ 8pt・primary に**（`CellRefSpans`）。**プレビューの色分けと余白**（`modPeek.PeekSpans`＋`Characters`）。チップの `…` と「ほか N件」。**作業用 Excel の案内文**を「Excel が2つあります／空の方が作業用／そちらだけ閉じる」（103字・約12秒）。
**波B（Hub・共通 UI）**: **accent/primary/danger の面に乗せる文字色を輝度で決める**（`onXxx`。白固定は gold primary 2.10・gold danger 2.77・dark danger 2.77 が AA 未達だった。**しきい値方式は light primary を 3.93 に誤判定するので、白と濃色の実コントラストを比べる方式**へ＝司令塔 Fix。全18通りで 4.5:1 以上を検算）。**Hub のバッジは獲得済みだけ濃く太字**（`modHubBadge` 新規・結合セルの左上へ `Characters`）。削除確認の既定を「いいえ」に。実行不可能な案内文4箇所・存在しない画面名3箇所・8pt→8.5pt 4箇所・ダッシュボードの単位・一覧400件の打ち切り案内・ヘッダー表記・ジャーゴン。旧ホーム画面の出典表記。PDF ページ読取失敗のログ。**パック取込のベクトル次元検査**（受入 PA-04/05）。
**波C（文書・配布）**: `docs/05` を R43 化（版表記・**Excel はシートN**・R42 の3ファイル・案内窓6種・事実の綻び2件・図の横スクロール）。**`docs/06_部門ナレッジの配り方ガイド.html`（新規）を zip へ同梱＝同梱7本**。`docs/46`/`docs/48` の訂正。**発行・巻き戻しの「自動で配信されます」を実態へ**（受け取り側が押さないと入らない）。bat の書き戻し時刻記録。「10分」の説明を3分割。
**波D（層1・GO 済）**: **Excel の番地を値の直前へ**（`[B42] 値`）。`RowTextFrom` に `rowIdx`/`baseCol` を **Optional で追加**（既存7箇所は無改変で互換）。**番地は値のあるセルだけ**（密な行 215→269字＝+25.1%／疎な行 51→57字）。`CitationInstruction` は字数を増やさず書き換え。**既存の Excel 資料は削除→再取込で新しい番地になる**（`docs/10` に案内）。

**レビュー（Opus 2周・Fix は司令塔）**: 1周目 MAJOR 5・MINOR 6 → 全件採用（M1 結論強調の除外2経路／M2 12pt 化に `AutoSize` 再フィットと下端取り直しが無い／M3 打ち切り案内が描画途中で3秒ブロックし一度も見えない／M4 文書の旧引用／M5 config 引き当てがループ内）。2周目 MAJOR 2・MINOR 7 → 全件採用。**2周目で 1周目 Fix の回帰を1件検出**: 空文字セルに番地を付けない修正が**行の存否まで巻き込み**、`R33 W3-1「数式ブランクの行は残す」`を壊していた（**LO の pure が実際に落ちて発覚**）。番地の有無と行の存否を分離し Pure48 に T1b/T1c を追加。
**検問（09-10・司令塔）**: lint ERROR 0／LO compile **178/178**／pure **PASS 3386 / FAIL 0 / SKIP 15**（下限 3386・上限 15。ネガティブ確認済み）／`check_launcher` OK／`--dev`・`--prod --zip` 自己検証 OK／`bin_roundtrip --book` 6条件／`lo_xlsm --book` 3条件 OK。**ネガティブ確認**: Pure48 の T1c を `[A7] ` に壊して **FAIL 1（PASS 3385）**→復元→3386 で緑。配布物 `dist/MyBookshelf_配布.zip`（build `20260910-035004Z+9fde83a`・14,732,697B・xlsm 2,020,448B・24シート・178本・`MyBookshelf.build` 同梱・**docs 7本**＝05/45/00/41/10/44/**06**・図解ガイドの版表記 **R43**・config 実測: `mock_llm` FALSE・キー空・`nexus_share_path` 空・`embed_dim` 768・`chunk_mode` structure・`onepass_max_chars` 450000・`thorough_onepass` on・`graph_outline` on・`pii_scan_enabled` FALSE）。**ユーザーへ送付済み。**
**実機受入（spec §5）**: **R41 分（Excel の出典が `シートN`＋番地）と R42 分（PDF の出典ページ・版上げ）も未実施のまま持ち越し。** R43: (1) 回答の冒頭の結論が大きく太字・`(B3 付近)` が出典タグと同じ小ささと色 (2) チップの吹き出しが色と余白で分かれる・長い資料名が `…`・5件以上で「ほか N件」 (3) Hub のバッジは獲得済みだけ濃く太字・**着せ替えを変えても送信ボタンの文字が読める**（特にエグゼクティブ・ゴールド） (4) 資料の削除確認で Enter を押しても消えない (5) 作業用 Excel の案内が「2つある・空の方が作業用」と読めて約12秒で消える (6) zip の `docs/06` を見ながら部門ナレッジを1回配れる。
**R44 送り（spec §6）**: 混在 PDF の未読ページ判定／取り込み済みの不正ベクトルの修復／旧版引き継ぎの事前検証／ランチャーの完成形／`modSkin.ApplyTheme` からの本文装飾の再適用（着せ替えでタグ色が戻る既存バグ）／ビュー切替ピルとシートタブの呼び名統一／`Range.Characters` の書式ラン上限（バッジ帯は現在177字・255字上限）／`OnAccentColor` の濃色が4テーマのパレットに無い色である点。
**容量（09-10 実測・危険水域）**: `modUIShelf` 残10 ／ `modBoard` 残19 ／ `modUINexusDraw` 残21 ／ `modUIMain` 残60 ／ `modChunker` 残76 ／ `modHubStat` 残77。**WARN を出さない 27,900〜28,000 の4本**: `modShelf` 残1・`modUtil` 残8・`modBoot` 残9・`modSkin` 残39（CLAUDE.md §12 に帯を新設）。受け皿: `modHubBadge` 27,110／`modLiveStyle` 18,188／`modChunkPage` 19,463／`modToast` 22,656。

## 0R42. R42（外部受入 FAIL 対応: PDF 出典ページ・版上げ bat・小修正・文書・2026-09-09・**検問全緑・実機受入待ち（R41 分も未実施）**）

**起点**: 外部の受入テスト報告（`docs/dev/acceptance_20260909/`・対象 R41 配布物 `db1d1af`・総合「受入不可」）。司令塔がソースで一件ずつ再現し、案1（層2全部＋A1＋B1＋B2）を提示 → ユーザー「全部任せる。GO」。全裁定 = `spec_20260909_R42_受入FAIL対応.md`（§0 裁定表・§0-2 レビュー裁定・§1 modChunkPage・§2 bat・§3 小修正・§4 文書・§5 実機確認・§6 R43 送り）。
**A1（最重要）**: 見出しの無い複数ページ PDF で全かけらの出典が p.1 に潰れていた（`ChunkAllPagesStructured` がブロック先頭ページを全かけらに付与）。→ 新 **`modChunkPage`**（`BlockAddPg`/`PlanWindows`/`FlushPaged`/`PhysicalKeep`/`StartPagesOf`＋Private `PrepareBlock`/`PagesForPlan`）。行ごとの物理ページを並走させ、**各かけらの本文先頭が載るページ**を付ける（本文の切り方は不変＝レビューが 4,000 ケースで一致確認）。modChunker は呼び替えのみ（残76）。**取込済みの PDF は再取込しないと直らない**（docs/10 に案内）。**A2**: 上限ページを物理番号に（先頭白紙で多く残る）。
**B（bat）**: 版の印 `MyBookshelf.build`（= config `build_stamp`・zip 同梱）。SRC/DST の印を `set /p` で比べ、違えば D: の本体を **D: の `_旧版` へ move し、成功後に OneDrive 側へ `_旧版_控え` を複製**（控えの失敗は案内のみで起動続行）してから新版を強制複製（引継ぎは既存の初回案内が D: の `_旧版` を見る。「いいえ」は二度と聞かない R36 契約のまま＝データは `_旧版`＋控えに残る）。同版は従来の `/D`。起動番号 `SID` 付きの `.closed.<SID>`。各コピーの成否検査（退避失敗は書き戻し中止・定期書き戻し失敗は1回案内）。Ghostscript は exe+dll 両方を見て、複製後も再確認。`tools/check_launcher.py`（7項目・ネガティブ確認済み）を final-gates に追加。
**C1** modPack の `On Error GoTo 0`→`LoadFailed`＋page 範囲検査（CDbl 経由）／**E1** modP2P 確定記録の書込み成功時のみ元票 GC／**F1** Pure2 の xlsx 出典期待 `シート2`（R41 の取り残し。LO は当該群 SKIP）／**F2** modTestsExcel は「別名同本文＝failed＋MEMO_ALL_DUP」の現行契約へ／**G** 同梱文書 10 件（存在しないリンク・約22MB・再起動手順・正典を発行 📤・publish_lock_stale と E0807・Ghostscript 欠損手順・「最大4回」撤回・取込中終了の2択）。
**レビュー**: 1周目（Opus）MAJOR 3・MINOR 9 → 採用 M1〜M3・m4〜m6・m8（Fix `688f352`・司令塔）／記録 m1（オーバーラップ起点の窓が前ページ扱い＝仕様どおり・既定値で 14.88%）・m2・m3・m9（spec §0-2）。2周目（Opus・Fix 検証）MAJOR 2・MINOR 7 → 採用 MAJOR-1（検査器の `if exist` 誤認）・MAJOR-2/m4/m5（退避順序＝move→控え別名・失敗は案内）・m6 縮退（`:BUILD_FAIL`）・m7（空の印）・m8（GS 案内1回）・m9（`PlanFor` 共通化＋Pure45 2検査）／記録 m3（spec §0-3・Fix2 は司令塔）。
**検問（09-09・司令塔・Fix2 後）**: lint ERROR 0／LO compile **173/173**／pure **PASS 3294 / FAIL 0 / SKIP 15**（下限 3294・上限 15。ネガティブ確認: Pure45 ゴールデンを "1,2,2,3,5" に壊して FAIL 1→復元、原子保持検査を壊して FAIL 1→復元）／`check_launcher` OK（7項目・(6)(7) をネガティブ確認）／`--dev`・`--prod --zip` 自己検証 OK（173本）／`bin_roundtrip --book` 6条件／`lo_xlsm --book` 3条件 OK。配布物 `dist/MyBookshelf_配布.zip`（build `20260909-051959Z+6e32834`・14,689,676B・xlsm 1,993,123B・bat 10,956B・`MyBookshelf.build` 26B＝config `build_stamp` と一致・docs 6本・Ghostscript 4ファイル・config 実測: `mock_llm` FALSE・キー空・`chunk_mode` structure・`onepass_max_chars` 450000・`thorough_onepass` on）。**ユーザーへ送付済み。**
**実機受入（spec §5）**: **R41 分（Excel の出典 `[本棚:x.xls シート1]`＋`(B3 付近)`・チップ `(シート1)`・行頭 `[B3]`）は未実施のまま持ち越し。** R42: (A1) 見出し無しの 5 ページ以上の PDF を取り込み、後半ページの語で質問→出典 p.4/p.5・チップで該当ページ (B1) R41 の配布フォルダへ R42 zip を上書き展開→bat 起動→「前の版から引き継ぎますか」→はい→本棚が戻る。D:\MyBookshelf と OneDrive 側に `MyBookshelf_旧版.xlsm`、D: に `MyBookshelf.build` (B2) 同版で再起動→聞かれず本棚が残る (B3) bat 二重起動→先を閉じても後の監視窓が終わらない (B4) dll を消して起動→補充。
**R43 候補（spec §6）**: A3 混在 PDF のページ別未読判定／C2 パックのベクトル検証／D1 旧版引継ぎの事前検証／m1 の帰属改善／`.closed.<SID>` 孤児掃除／旧ホームシートの `p.`。
**容量（09-09 Fix 後 python len）**: modChunkPage 10,008（新規）／modChunker 29,924（残76）／modExtractor 29,732（残268）／modTestsPure2 29,798（残202）／modPack 26,204／modP2P 27,865／Pure45 13,367。
...
**F1 の中核（Shell／kernel32／完了フラグ）は LO で検査できない＝実機報告だけが検証手段。** 再発したら Windows セキュリティ→保護の履歴の脅威名、usage_log の `gs_kill_unavailable`、`%TEMP%\nxocr_*\gs_out.log` と `done.flag` の中身（spec §4-1）。**伝書鳩（リスクコンサル PJ・09-08 返信）**: 向こうは外部プロセス非起動で無影響・教訓は docs/24 に記録済み。**こちらが会社 PC で通ったら「AV 警告の有無・Defender のバージョン」を送る**（向こうの Declare 禁止を緩める判断材料）。spec §4-1 に控え項目を追記済み。
**実機受入（09-08 ユーザー報告）**: **F1 緑**（同じ PDF が取り込めた・Defender の警告・通知は一切出ず）。Defender の版は会社 PC の制限でバージョン情報画面自体が見られない（不明のまま返す）。F5 は体感変化なし（450000 据え置き・遅くなったら 300000）。F2〜F4 は未報告。**新規報告（R41 起点）**: トーストが消えないことが多数（waitless 型 13 箇所は自動消去が無い設計＝原因確定）／フォルダ同期中に取込中の表示（進捗バナー）が途中で消える（`modEmbed` AfterLoop の `HideProgress` がループ所有のバナーを各ファイルの末尾で消す＝原因確定）。裁定・GO 待ち。
...
（モデルは gpt-5.5【R34当時の値。R48 で luna/terra/sol の3段へ差し替え済み】・チャットは社内リボン`ChatGPT()`経由でAPI直叩きではない・UI提案4件は既存実装済み）。
...
> ⚠️ **この節より上の「最新ラウンド節」（現在は §0R43）が現在地の正。**
