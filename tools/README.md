...
(`Split(...)(0)`のような即時インデックス参照は対象外)。If/For等の制御構文・
宣言文はそもそも呼び出し文として扱わない。2026-08-06(R19H FB-3)に、
`Call Foo(Split(...))`(先頭の`Call`を剥がして再解析)と、代入右辺の関数呼び出し
`n = Foo(Split(...))`(左辺は見ず`=`の右側だけを走査。入れ子も拾う)を対象へ
足した。実VBAはどちらの形も同じ理由でコンパイルを拒否する。文字列リテラルの
中身は走査前に空白へ潰す(`s = "Foo(Split(x))"`を誤検知しない)。名前付き引数と
単一行`If`は現リポ0件のため未対応(記録のみ)。呼び先が
