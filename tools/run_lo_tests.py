...
#   2026-09-19 R48 Fix(敵対的レビュー R48-REV-01〜04 の手直し): modTestsPure47 の
#     R群へ R23〜R29(新エラーコード E0205/E0206/E0207 の FriendlyMessage が
#     「予期しない問題」の既定値へ落ちていないこと・3つが互いに違う文言であること・
#     未定義コードは既定値へ落ちること)を追加 → 実走で 3494→3501(+7)。
#     ネガティブ確認済み: modLog の Case "E0207" を一時的に外して
#     R25_e0207_not_fallback が FAIL 1(実際=False 期待=True)になることを確認 → 復元。
#   2026-09-19 R48 Fix2(レビュー2周目): modTestsPure47 に R30(中断の案内が
#     modAsk.bas:249 の Err 18 分岐と逐語で同一であること)を追加 → 3501→3502。
#     ESC は「段の境界」と「LLM呼び出しの内側」の2経路で観測されるが、利用者に
#     とっては同じ操作なので、違う案内を出してはならない(初版は操作指示が
#     食い違っていた)。ネガティブ確認済み: modLog 側を「中断」→「中止」と
#     1文字だけ変えて FAIL 1 になることを確認 → 復元。
#   2026-09-19 R48 Fix3(3周目): modTestsPure47 に R31〜R35(中断の伝播 IsCancelled が
#     E0205「通信失敗」や E0204「利用上限」を中断と取り違えないこと)を追加 → 3502→3507。
#     取り違えると【直すべき障害が「利用者が止めた」ことにされて消える】。
#     ネガティブ確認済み: IsCancelled を「#ERR なら何でも中断」に緩めて
#     R32/R33 が FAIL 2 になることを確認 → 復元。
#   2026-09-19 R49(監査 H-7): modTestsPure47 に S群(modInsightGate.PiiScanHit・
#     共有フォルダへ出す前の個人情報走査)を9件追加 → 3507→3516。
#     解決済みQ&A(EmitVerifiedQA)は質問全文・回答全文・実名の3点セットを
#     部内へ置く経路なのに、3つある発信経路のうちここだけ走査が1度も
#     無かった。陽性2件だけでなく【陰性の反例7件】を必ず置いている ――
#     この関門は過検知すると部内ナレッジのフライホイールを止めるので、
#     「日時を書いただけの質問」「約款の条番号・金額」「改行で分かれた
#     数字」が通ることまで固定しないと、次に閾値を触った人が静かに壊す。
#     ネガティブ確認済み: S1(携帯番号)の期待値を True→False に壊して
#     FAIL 1(R48-47-S1_携帯番号 実際=True 期待=False)になることを確認 → 復元。
EXPECTED_PASS_MIN = 3516
...
PURE_ALLOWLIST = [
    ...
    # modClip(2026-09-11 R46・実機報告「1つ前の出力内容がコピーできない」):
    #   テストが呼ぶのは1行見出しの純関数 HeadLineOf だけ。OnCopyPastAnswer /
    #   CollectAiBubbles は Shape と modUiLock / Application.InputBox に触るので
    #   テストからは呼ばない(modPeek の ShowPeek/DrawChip と同型)。それらが
    #   参照する未注入モジュールは「実行に到達しない未定義識別子」としてLOの
