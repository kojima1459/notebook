...
- **2026-09-11 R46 実測での訂正（上の R43 台帳は古い値を含む）**: **`modExtractorExcel` は 11,372 ではなく 19,714 字（残10,286）** / **`modGenPipe` の残りは 8,014 ではなく 6,873（R45 で `CarryRules` を足した後）**。R46 で触った結果: `modPeek` 残2,335（受け皿から逼迫手前へ）／`modKnowledgeBar` 残2,562／`modClip` 残21,023／`modToast` 残21,471／`modProgressBar` 残17,279／`modLiveStyle` 残17,180／`modAppState` 残9,159／`modMode` 残833／`modSetupWizard` 残23,765／**`modHelp` 残1,687（R46 でヘルプカード7段目を足し WARN 帯へ入った）**。
  R46 の A-4（確信度の作り直し）後: `modPeek` 残2,335 ／ `modTestsPure48` 残3,891。**台帳の数字は触るたびに python の `len` で測り直すこと。伝聞のまま持ち回ると、実装班が「入る」と判断して入らない。**
- **2026-09-18 R48 実測（上の R43/R46 台帳より新しい。触ったものだけ）**: **`modGateway` 残1,289**（`LooksLikeLimitError`/`LooksLikeRealAnswer` を新設 `modRibbonFail` へ出して**残125 → 残1,289 へ回復**。§12 の分割裁定を実行した結果） ／ **`modRibbonFail` 残19,656（R48 新規・受け皿。リボンが返す失敗の語彙の単一情報源）** ／ **`modAsk` 残300（凍結。分岐順序の1行コメントだけ）** ／ **`modMode` 残833（`WorkingNote` を収めた。受け皿ではなくなっている。**準逼迫**）** ／ **`modAskRetrieve` 残94（モデル名の既定文字列を伸ばした。ここへはもう1文字も入らない）** ／ `modAskMulti` 残1,791 ／ `modAskOnePass` 残8,239 ／ `modAskGlobal` 残7,757 ／ `modAskThorough` 残11,772 ／ `modLog` 残3,774（E0205/E0206/E0207 を追加） ／ `modDiag` 残8,054 ／ `modEmbed` 残12,264 ／ `modEnrich` 残13,580 ／ `modOutlineBuild` 残2,944 ／ `modSynonymStore` 残15,389 ／ `modAppState` 残9,159 ／ `modGenPipe` 残6,868 ／ `optVision` 残1,693 ／ `optOcrPage` 残1,762 ／ `optDiffDoc` 残19,519 ／ `modTestsPure47` 残17,261 ／ `modTestsPure18` 残372 ／ `modTestsPure29` 残3,302。
