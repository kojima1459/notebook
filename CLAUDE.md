...
- **実質凍結（残100字未満・2026-09-09 R42 後 python len 実測）**: **`modBoard`19** / `modUINexusDraw`37 / **`modChunker`76**（R42 でページ計算の実体を `modChunkPage` へ。呼び替えだけ残す） / **`modHubStat`77** / **`modUIShelf`79**（R36 で `AfterShot` 引数 +14 字） / **`modUIMain`83**（R41 で `OneSourceLabel` の `p.` を見送った理由）。**1行も入らない**。`modSkin` は R41 でトースト一式を `modToast` へ分割し残 2,258 に回復（実質凍結から外れた）。
- **逼迫（残300字未満＝分割裁定必須・2026-09-09 R42 実測）**: `modViewport`118 / `modGateway`125 / **`modTestsPure2`202**（R42 F1 で注記＋期待値。テストは Pure45 以降へ） / **`modExtractor`268**（R42 A2 で `PhysicalKeep` 経由。**次に触るなら分割が先**） / **`modAskRetrieve`126**（R37 で `RememberPool`/`Expand` の2行。**次に触るなら分割が先**） / **`modShare`186**（実体はmodChatLog等へ・自分は1行呼び出しの型を維持） / **`modBackdrop`197** / `modClarify`233 / **`modApp`239**（R41 で ScreenUpdating 1行＋waitless 引数） / **`modUI`258**（R41 で `modToast.CancelSweep` 1行） / `modVaultGallery`286。
- **準逼迫（残1,000字未満・次に触るときは要注意・2026-09-09 R42 実測）**: `modChrome`350 / `modChannel`389 / **`modKnowledge`416** / `modShelfStore`431 / **`modHub`449** / **`modUtilText`584** / `modIntegrity`666 / `modViewport2`688 / **`modAsk`732（凍結。次に手術するなら分割が先）** / `modSparse`953 / **`optOcrCore`1,331（R40 F1 で BuildRunCommand を -sstdout 挿入に・ヘッダ整理。2026-09-08 実測。次に触るなら読み取り側を新モジュールへ）** / **`modTestsPure6`1,212（R40 でゴールデン更新）** / **`modShelf`2,001（凍結）** / **`modShareRule`1,238** / **`modShelfBatch`360**（R41 で SetLoopBanner/LoopBannerOwned＋Fix2 の失効 AND・**次に触るなら分割が先**） / `modUtil`2,008 / **`modPrompts`1,841（凍結。R41 で SourceTag/CitationInstruction の2関数だけ解除・WARN 圏）** / **`modLive`2,175**（R41 で TagCloseOk） / `modInsightIo`2,365。テスト側は `modTestsPure24`347 / `modTestsPure25`365 / `modTestsPure18`402 / `modTestsPure2`451 / `modTestsPure34`705 / `modTestsPure35`723 / `modTestsPure11`738 / `modTestsPure33`997。
- **受け皿（実測残り・2026-09-09 R42）**: **`modChunkPage`19,992**（R42 新規。構造チャンクのかけら別ページ計算・PhysicalKeep。modChunker の受け皿） / **`modToast`22,656**（R41 新規。トースト一式・OnTime 自動消去） / **`modMode`8,637**（R41 で PageTagPart） / **`modMigrate`8,604**（R36 波4で CopyUserData を modMigrateFrom へ移して受け皿化） / **`modMigrateFrom`6,236** / **`modCorrect`4,029**（R37 で降格を足して受け皿を脱しつつある） / **`modTextView`14,493** / **`modXDoc`15,858** / **`modXDocBuild`8,623** / **`modXDocStore`5,258** / `modOutlineBuild`2,955 / `modAppAct`10,397 / `modAppState`10,173 / `modKnowledgeBar`5,479 / `modTypes`28,831 / `modAppDef`27,268 / `modState`26,645 / `modClip`25,603 / `modFeatures`24,307 / **`modChatLog`23,490**（R34からboard系.bak掃除の実体も同居） / `modGatewayDirect`22,314 / `modConfig`21,980 / `modInstallCheck`26,854（R35波1手術でVI系削除→受け皿化。2026-09-03実測） / `modProgressBar`17,542 / `modInsightGate`14,668 / `modExtractorExcel`14,347 / `modAppState`13,810→12,943 / `modStarter`13,389 / **`modTelemetry`11,896** / `modPublishUI`10,536 / `modMode`9,613 / `modGuard`9,342 / `modGenPipe`8,014 / `modDiag`8,097 / `modDash`5,879 / `modShared`3,448。
