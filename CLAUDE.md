...
- **2026-09-18 R48 実測（上の R43/R46 台帳より新しい。触ったものだけ）**: **`modGateway` 残1,289**（`LooksLikeLimitError`/`LooksLikeRealAnswer` を新設 `modRibbonFail` へ出して**残125 → 残1,289 へ回復**。§12 の分割裁定を実行した結果） ／ **`modRibbonFail` 残20,176（R48 新規・受け皿。リボンが返す失敗の語彙の単一情報源）** ／ **`modAsk` 残300（凍結。分岐順序の1行コメントだけ）** ／ **`modMode` 残833（`WorkingNote` を収めた。受け皿ではなくなっている。**準逼迫**）** ／ **`modAskRetrieve` 残94（モデル名の既定文字列を伸ばした。ここへはもう1文字も入らない）** ／ `modAskMulti` 残2,982 ／ `modAskOnePass` 残8,239 ／ `modAskGlobal` 残8,130 ／ `modAskThorough` 残12,730 ／ `modLog` 残4,178（E0205/E0206/E0207 を追加） ／ `modDiag` 残8,054 ／ `modEmbed` 残12,264 ／ `modEnrich` 残13,580 ／ `modOutlineBuild` 残2,944 ／ `modSynonymStore` 残15,389 ／ `modAppState` 残9,159 ／ `modGenPipe` 残6,868 ／ `optVision` 残1,693 ／ `optOcrPage` 残1,762 ／ `optDiffDoc` 残19,519 ／ `modTestsPure47` 残19,488 ／ `modTestsPure18` 残372 ／ `modTestsPure29` 残3,302。
